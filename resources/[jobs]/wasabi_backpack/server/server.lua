...
AddEventHandler('wasabi_backpack:openBackpack', function(identifier)
	if not registeredStashes[identifier] then
        ox_inventory:RegisterStash('bag_'..identifier, 'Bewijszakje', Config.BackpackStorage.slots, Config.BackpackStorage.weight, false)
...
lib.callback.register('wasabi_backpack:getNewIdentifier', function(source, slot)
	...
	ox_inventory:RegisterStash('bag_'..newId, 'Bewijszakje', Config.BackpackStorage.slots, Config.BackpackStorage.weight, false)
...
CreateThread(function()
	...
	local swapHook = ox_inventory:registerHook('swapItems', function(payload)
		...
		local count_bagpacks = ox_inventory:GetItem(payload.source, 'bewijstas', nil, true)
	...
	if Config.OneBagInInventory then
		createHook = exports.ox_inventory:registerHook('createItem', function(payload)
			local count_bagpacks = ox_inventory:GetItem(payload.inventoryId, 'bewijstas', nil, true)
			...
			if count_bagpacks > 0 then
				...
				Citizen.CreateThread(function()
					...
					for i,k in pairs(ox_inventory:GetInventoryItems(inventoryId)) do
						if k.name == 'bewijstas' and dontRemove ~= nil and k.slot ~= dontRemove then
							local success = ox_inventory:RemoveItem(inventoryId, 'bewijstas', 1, nil, k.slot)
							if success then
...
exports('logArmour', function(event, data, data1)
    local xPlayer = ESX.GetPlayerFromId(source)
		TriggerEvent(
                ...
                xPlayer.source,
                {
                    title = GetPlayerName(xPlayer.source) .. ' heeft zojuist armor gebruikt!',
                    desc = "Job: Politie"
                 },
                0x000001
            )
