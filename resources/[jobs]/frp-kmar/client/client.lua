...
RegisterNetEvent('frp-kmar:client:people:shooted')
AddEventHandler('frp-kmar:client:people:shooted', function(label, shooterCoords)
...
RegisterNetEvent('frp-kmar:client:noodknop')
AddEventHandler('frp-kmar:client:noodknop', function(name, coords)
...
Citizen.CreateThread(function()
	...
	exports['qtarget']:Player({
		options = {
			{
				event = 'frp-kmar:client:give:fine',
	...
	exports['qtarget']:Vehicle({
		options = {
			{
				event = 'frp-kmar:client:shut:vehicle',
				...
				for k,v in pairs(weapons) do
					if GetHashKey(v.name) == weapon then
						if IsPedArmed(ped, 4) then
							TriggerServerEvent('frp-kmar:server:people:shooted', v.name, 'Vuurwapen', shooterCoords)
							break
						end
					end
				end
...
OpenVehiclesMenu = function(type, configType, data)
	...
	for i=1, #configType[data.category].vehicles do 
		vehicles[#vehicles+1] = {
			...
			event = 'frp-kmar:client:spawn:vehicle',
...
RegisterNetEvent('frp-kmar:client:open:stash')
AddEventHandler('frp-kmar:client:open:stash', function()
...
GetGear = function()
		local options = {
			...
			[5] = {
				...
				['event'] = 'frp-kmar:client:delete:weapon:menu',
            	['icon'] = 'fa-sold fa-gun'
			}
		}
...
RegisterNetEvent('frp-kmar:client:own:cloakroom')
AddEventHandler('frp-kmar:client:own:cloakroom', function()
...
RegisterNetEvent('frp-kmar:client:kmar:cloakroom')
AddEventHandler('frp-kmar:client:kmar:cloakroom', function()
	if ESX.PlayerData.job.name == 'kmar' or ESX.PlayerData.job.name == 'offkmar' then
		exports['frp-jobsmenu']:OpenOutfitMenu(Config.Outfits)
	end
end)

RegisterNetEvent('frp-kmar:client:kmar:cloakroom:shop')
AddEventHandler('frp-kmar:client:kmar:cloakroom:shop', function()
...
RegisterNetEvent('frp-kmar:client:buy:weapon')
AddEventHandler('frp-kmar:client:buy:weapon', function()
	exports.ox_inventory:openInventory('shop', {type = 'kmarkluis'})
end)

RegisterNetEvent('frp-kmar:client:buy:bsbweapon')
AddEventHandler('frp-kmar:client:buy:bsbweapon', function()
	if ESX.PlayerData.job.name == 'kmar' then
...
RegisterNetEvent('frp-kmar:client:buy:hrbweapon')
AddEventHandler('frp-kmar:client:buy:hrbweapon', function()
	if ESX.PlayerData.job.name == 'kmar' then
...
RegisterNetEvent('frp-kmar:client:buy:rechercheweapon')
AddEventHandler('frp-kmar:client:buy:rechercheweapon', function()
	...
	if ESX.PlayerData.job.name == 'kmar' or ESX.PlayerData.job.name == 'offkmar' then
		local options = {
			...
			[2] = {
				...
				['event'] = 'frp-kmar:client:kmar:cloakroom:shop'
			}
		}
	...
	while true do
		...
		if ESX.PlayerData.job.name == 'kmar' then
			...
			if IsControlJustPressed(0, 57) then
				...
				if count >= 2 then
					TriggerServerEvent('frp-kmar:server:noodknop')
...
RegisterNetEvent('frp-kmar:client:spawn:vehicle')
AddEventHandler('frp-kmar:client:spawn:vehicle', function(data)
...
RegisterNetEvent('frp-kmar:client:search:player')
AddEventHandler('frp-kmar:client:search:player', function(data)
...
RegisterNetEvent('frp-kmar:client:shut:vehicle')
AddEventHandler('frp-kmar:client:shut:vehicle', function(data)
	local entity = data.entity
	local ped = PlayerPedId()
	...
	TriggerServerEvent('frp-kmar:server:unlock:vehicle', NetworkGetNetworkIdFromEntity(entity))

	SetVehicleDoorsLockedForAllPlayers(entity, false)
end)

RegisterNetEvent("frp-kmar:client:delete:weapon:menu")
AddEventHandler("frp-kmar:client:delete:weapon:menu", function()
	...
	for k, v in pairs(items) do
		if string.lower(v.name):match("^weapon_") then
			table.insert(options, {
				...
				serverEvent = 'frp-kmar:server:delete:weapon:menu',
...
RegisterNetEvent('frp-kmar:client:give:fine')
AddEventHandler('frp-kmar:client:give:fine', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
	...
	if distance < 2.0 then
		local options = {
			{
				...
				['event'] = 'frp-kmar:client:search:fine:list',
				['args'] = { entity = entity }
			}
		}
		...
		for k,v in pairs(Config.Fines) do
			options[#options+1] = {
				title = k,
				event = 'frp-kmar:client:give:fine:list',
...
RegisterNetEvent('frp-kmar:client:search:fine:list')
AddEventHandler('frp-kmar:client:search:fine:list', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
		...
		if input then 
			...
			for i=1, #found do
				options[#options+1] = {
					...
					event = 'frp-kmar:client:give:fine:finalise',
...
RegisterNetEvent('frp-kmar:client:give:fine:list')
AddEventHandler('frp-kmar:client:give:fine:list', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
...
RegisterNetEvent('frp-kmar:client:give:fine:finalise')
AddEventHandler('frp-kmar:client:give:fine:finalise', function(data)
...
exports.ox_target:addSphereZone({
  ...
  options = {
      {
          name = 'box',
          event = 'frp-kmar:client:analyse:fingerprint',
...
RegisterNetEvent('frp-kmar:client:analyse:fingerprint', function () 
    ...
    TriggerServerEvent('frp-kmar:server:analyse:fingerprint', netId)
end)

RegisterNetEvent('frp-kmar:client:show:fingerprint')
AddEventHandler('frp-kmar:client:show:fingerprint', function(data)
      ...
      if target ~= -1 and distance <= 3.0 then
        ...
        if DoesEntityExist(targetPed) then
		  ...
		  elseif data.current.value == "revive" then
			...
			return end
			TriggerServerEvent('frp-kmar:server:remove:item')
			...
			elseif data.current.value == "id_card" then
				...
				local networkPlayer = NetworkGetPlayerIndexFromPed(playerPed)
				local playerId = GetPlayerServerId(networkPlayer)
				TriggerServerEvent('frp-kmar:server:get:id', playerId)
			elseif data.current.value == "id_card_card" then
				...
				TriggerServerEvent('frp-kmar:server:get:id:id', playerId)
			end
        else
...
RegisterNetEvent('frp-kmar:client:show:id')
AddEventHandler('frp-kmar:client:show:id', function(data)
    lib.registerContext({
        id = 'identiteitkaart',
...
RegisterNetEvent('frp-kmar:client:show:id:id')
AddEventHandler('frp-kmar:client:show:id:id', function(data)
    lib.registerContext({
        id = 'rijbewijzen_id',
...
Checkpersons = function()
    ...
    ESX.TriggerServerCallback("frp-kmar:check:gangmembers", function(datagang)
...
function PromotePlayer(identifier, playerName)
    TriggerServerEvent("frp-kmar:promotemember", identifier, playerName)
end

-- Example of how to trigger the demotion event from client-side
function DemotePlayer(identifier, playerName)
    TriggerServerEvent("frp-kmar:demote", identifier, playerName)
end


function Deletemembersboss(x, y)
	TriggerServerEvent("frp-kmar:deletemember:serversided", x, y)
end
...
function Neempersonenaan()
	...
	if playerid then
		ESX.TriggerServerCallback('frp-kmar:add:playertogang', function(done)
			if done then
			end
