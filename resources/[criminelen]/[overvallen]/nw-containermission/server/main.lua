...
-- MELDING VOOR SPELERS OM DE LOCATIE TE VINDEN
local notificationInterval = 3600 * 1000 -- 1 hour in milliseconds

local function sendRandomNotification()
    local players = ESX.GetPlayers()
    local playerCount = #players
    local selectedCount = math.ceil(playerCount * 0.1)

    if playerCount > 0 and selectedCount > 0 then
        local selectedPlayers = {}
        
        while #selectedPlayers < selectedCount do
            local randomPlayer = players[math.random(#players)]
            if not selectedPlayers[randomPlayer] then
                table.insert(selectedPlayers, randomPlayer)
            end
        end

        for _, playerId in ipairs(selectedPlayers) do
            TriggerClientEvent('okokNotify:Alert', playerId, 'Informatie', "Pssst, er is informatie binnengekomen over een nieuwe manier om geld te verdienen. Vind de npc rondom postcode 2027 om hier meer informatie over te vinden.", 5000, 'info')
        end
    end
end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(notificationInterval)
