...
local function CheckIfPlayerHasWeapon(xPlayer)
    for _, weaponName in ipairs(Config.RequiredWeapons) do
        local get_weapon_player = xPlayer.getInventoryItem(weaponName)

        if get_weapon_player and get_weapon_player.count >= 1 then
            return true
        end
    end
    return false 
