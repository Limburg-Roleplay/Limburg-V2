...
RegisterServerEvent('lrp-kmar:server:people:shooted')
AddEventHandler('lrp-kmar:server:people:shooted', function(weaponname, cat, coords)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name == 'kmar' then
        TriggerClientEvent('lrp-kmar:client:people:shooted', -1, cat, coords)
    end
end)
RegisterServerEvent('lrp-kmar:server:remove:item')
AddEventHandler('lrp-kmar:server:remove:item', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.job.name == 'kmar' then
...
RegisterServerEvent('lrp-kmar:server:delete:weapon:menu')
AddEventHandler('lrp-kmar:server:delete:weapon:menu', function(weaponName)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name == 'kmar' then
...
RegisterServerEvent('lrp-kmar:server:get:id:id')
AddEventHandler('lrp-kmar:server:get:id:id', function(playerId)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if xPlayer.job.name == 'kmar' then
        ...
        MySQL.Async.fetchAll("SELECT type FROM user_licenses WHERE owner = @identifier", { ["@identifier"] = Identifier }, function(result)
			...
			xPlayer.triggerEvent('lrp-kmar:client:show:id:id', buildData)
        end)
    else
...
RegisterServerEvent('lrp-kmar:server:get:id')
AddEventHandler('lrp-kmar:server:get:id', function(playerId)
    local xPlayer = ESX.GetPlayerFromId(source)
    local xTarget = ESX.GetPlayerFromId(playerId)
    ...
    if xPlayer.job.name == 'kmar' then
        ...
        MySQL.Async.fetchAll("SELECT firstname, lastname, sex, dateofbirth FROM users WHERE identifier = @identifier", { ["@identifier"] = Identifier }, function(result)
            xPlayer.triggerEvent('lrp-kmar:client:show:id', result[1])
        end)
    else
        exports["fiveguard"]:fg_BanPlayer(source, "heeft geprobeerd een id te bekijken zonder kmar job!", true)
    end
...
RegisterServerEvent('lrp-kmar:server:analyse:fingerprint')
AddEventHandler('lrp-kmar:server:analyse:fingerprint', function(playerId)
    local xPlayer = ESX.GetPlayerFromId(source)
    local xTarget = ESX.GetPlayerFromId(playerId)
    ...
    if xPlayer.job.name == 'kmar' then
        local Identifier = ESX.GetPlayerFromId(playerId).identifier
        MySQL.Async.fetchAll("SELECT firstname, lastname, sex, dateofbirth FROM users WHERE identifier = @identifier", { ["@identifier"] = Identifier }, function(result)
            xPlayer.triggerEvent('lrp-kmar:client:show:fingerprint', result[1])
        end)
    else
        exports["fiveguard"]:fg_BanPlayer(source, "heeft geprobeerd een id te bekijken zonder kmar job!", true)
    end
...
RegisterServerEvent('lrp-kmar:server:noodknop')
AddEventHandler('lrp-kmar:server:noodknop', function(weaponname, cat, coords)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if xPlayer.job.name == 'kmar' then
        GetRPName(source, function(Firstname, Lastname)
            ...
            TriggerClientEvent('lrp-kmar:client:noodknop', -1, volledig, coords)
        end)
    end
end)

RegisterServerEvent('lrp-kmar:server:buy:weapon')
AddEventHandler('lrp-kmar:server:buy:weapon', function(a)
...
RegisterServerEvent("lrp-kmar:promotemember")
AddEventHandler(
    "lrp-kmar:promotemember",
...
RegisterServerEvent("lrp-kmar:demote")
AddEventHandler(
    "lrp-kmar:demote",
    function(identifierplayer, voornaamplayer)
        local src = source
...
RegisterServerEvent("lrp-kmar:deletemember:serversided")
AddEventHandler(
    "lrp-kmar:deletemember:serversided",
    function(identifierplayer, voornaamplayer)
        local src = source
        local xPlayer = ESX.GetPlayerFromId(src)
        local playerHex = string.gsub(tostring(GetPlayerIdentifier(src)), "license:", "")
