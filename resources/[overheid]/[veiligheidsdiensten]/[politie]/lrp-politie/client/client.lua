...
RegisterNetEvent('lrp-politie:client:people:shooted')
AddEventHandler('lrp-politie:client:people:shooted', function(label, shooterCoords)
...
RegisterNetEvent('lrp-politie:client:noodknop')
AddEventHandler('lrp-politie:client:noodknop', function(name, coords)
...
Citizen.CreateThread(function()
	...
	exports['qtarget']:Player({
		options = {
			{
				event = 'lrp-politie:client:give:fine',
	...
	exports['qtarget']:Vehicle({
		options = {
			{
				event = 'lrp-politie:client:shut:vehicle',
...
Citizen.CreateThread(function()
	...
	local weapons = exports['lrp-logger']:getWeapons()
	while true do
		local sleep = 1
		local ped = PlayerPedId()
			...
			if weapon ~= GetHashKey('WEAPON_STUNGUN') and weapon ~= GetHashKey('weapon_fireextinguisher') and weapon ~= GetHashKey('WEAPON_UNARMED') then
				...
				-- 	if tostring(k) == tostring(weapon) then
						if IsPedArmed(ped, 4) then
							TriggerServerEvent('lrp-politie:server:people:shooted', weapon, 'Vuurwapen', shooterCoords)
							break
						end
				-- 	end
				-- end
...
OpenVehiclesMenu = function(type, configType, data)
	...
	for i=1, #configType[data.category].vehicles do 
		vehicles[#vehicles+1] = {
			...
			event = 'lrp-politie:client:spawn:vehicle',
...
RegisterNetEvent('lrp-politie:client:open:stash')
AddEventHandler('lrp-politie:client:open:stash', function()
...
GetGear = function()
		local options = {
			[1] = {
				...
				['event'] = 'lrp-politie:client:buy:weapon',
				['icon'] = 'fa-solid fa-gun'
			},
			[2] = {
				['title'] = 'DSI | Uitrusting Kluis',
				['description'] = 'Bekijk de DSI uitrusting kluis',
				['event'] = 'lrp-politie:client:buy:dsiweapon',
				['icon'] = 'fa-solid fa-gun',
			},
        	[3] = {
			...
			[4] = {
				...
				['event'] = 'lrp-politie:client:delete:weapon:menu',
            	['icon'] = 'fa-sold fa-gun'
			}
		}
...
RegisterNetEvent('lrp-politie:client:own:cloakroom')
AddEventHandler('lrp-politie:client:own:cloakroom', function()
...
RegisterNetEvent('lrp-politie:client:police:cloakroom')
AddEventHandler('lrp-politie:client:police:cloakroom', function()
	if ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'offpolice' then
		exports['lrp-jobsmenu']:OpenOutfitMenu(Config.Outfits)
	end
end)

RegisterNetEvent('lrp-politie:client:police:cloakroom:shop')
AddEventHandler('lrp-politie:client:police:cloakroom:shop', function()
...
RegisterNetEvent('lrp-politie:client:buy:weapon')
AddEventHandler('lrp-politie:client:buy:weapon', function()
	exports.ox_inventory:openInventory('shop', {type = 'politiekluis'})
end)

RegisterNetEvent('lrp-politie:client:buy:dsiweapon')
AddEventHandler('lrp-politie:client:buy:dsiweapon', function()
...
RegisterNetEvent('lrp-politie:client:buy:rechercheweapon')
AddEventHandler('lrp-politie:client:buy:rechercheweapon', function()
	...
	if ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'offpolice' then
		local options = {
			[1] = {
				...
				['event'] = 'lrp-politie:client:police:cloakroom'
			},
			[2] = {
				['title'] = 'Kleding Menu',
				['description'] = 'Open een Kleding Menu zoals in de winkel.',
				['event'] = 'lrp-politie:client:police:cloakroom:shop'
			}
		}
	...
	while true do
		...
		if ESX.PlayerData.job.name == 'police' then
			...
			if IsControlJustPressed(0, 57) then
				...
				if count >= 2 then
					TriggerServerEvent('lrp-politie:server:noodknop')
...
RegisterNetEvent('lrp-politie:client:spawn:vehicle')
AddEventHandler('lrp-politie:client:spawn:vehicle', function(data)
		...
		for i=1, #garage.spawnPoints, 1 do 
			if ESX.Game.IsSpawnPointClear(vector3(garage.spawnPoints[i].x, garage.spawnPoints[i].y, garage.spawnPoints[i].z), 2.0) then 
				ESX.Game.SpawnVehicle(vehicle, vector3(garage.spawnPoints[i].x, garage.spawnPoints[i].y, garage.spawnPoints[i].z), garage.spawnPoints[i].w, function(veh)
					...
					exports['lrp-carkeys']:giveCarKeys(GetPlayerServerId(PlayerPedId()), GetVehicleNumberPlateText(veh), ESX.Game.GetVehicleProperties(veh))
	...
	elseif data.garageIndex == 2 then --- Garage bij Plezierhaven
		if ESX.Game.IsSpawnPointClear(vector3(-745.1712, -1501.7766, 5.1236), 2.0) then 
			ESX.Game.SpawnVehicle(vehicle, vector3(-745.1712, -1501.7766, 5.1236), 23.01, function(veh)
				TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
				SetVehicleNumberPlateText(veh, props.plate)
				exports['lrp-carkeys']:giveCarKeys(GetVehicleNumberPlateText(veh), ESX.Game.GetVehicleProperties(veh))
				exports['lrp-benzine']:setFuel(veh, 100.0)
				TriggerServerEvent('lrp-garage:server:set:distanceculling', NetworkGetNetworkIdFromEntity(veh))
			end, true, props.plate)
			foundPoint = true 
...
RegisterNetEvent('lrp-politie:client:search:player')
AddEventHandler('lrp-politie:client:search:player', function(data)
...
RegisterNetEvent('lrp-politie:client:shut:vehicle')
AddEventHandler('lrp-politie:client:shut:vehicle', function(data)
	local entity = data.entity
	local ped = PlayerPedId()
	...
	TriggerServerEvent('lrp-politie:server:unlock:vehicle', NetworkGetNetworkIdFromEntity(entity))

	SetVehicleDoorsLockedForAllPlayers(entity, false)
end)

RegisterNetEvent("lrp-politie:client:delete:weapon:menu")
AddEventHandler("lrp-politie:client:delete:weapon:menu", function()
	...
	for k, v in pairs(items) do
		if string.lower(v.name):match("^weapon_") then
			table.insert(options, {
				...
				serverEvent = 'lrp-politie:server:delete:weapon:menu',
...
RegisterNetEvent('lrp-politie:client:give:fine')
AddEventHandler('lrp-politie:client:give:fine', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
	...
	if distance < 2.0 then
		local options = {
			{
				...
				['event'] = 'lrp-politie:client:search:fine:list',
				['args'] = { entity = entity }
			}
		}
		...
		for k,v in pairs(Config.Fines) do
			options[#options+1] = {
				title = k,
				event = 'lrp-politie:client:give:fine:list',
...
RegisterNetEvent('lrp-politie:client:search:fine:list')
AddEventHandler('lrp-politie:client:search:fine:list', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
		...
		if input then 
			...
			for i=1, #found do
				options[#options+1] = {
					...
					event = 'lrp-politie:client:give:fine:finalise',
...
RegisterNetEvent('lrp-politie:client:give:fine:list')
AddEventHandler('lrp-politie:client:give:fine:list', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
...
RegisterNetEvent('lrp-politie:client:give:fine:finalise')
AddEventHandler('lrp-politie:client:give:fine:finalise', function(data)
...
exports.ox_target:addSphereZone({
  ...
  options = {
      {
          name = 'box',
          event = 'lrp-politie:client:analyse:fingerprint',
...
RegisterNetEvent('lrp-politie:client:analyse:fingerprint', function () 
    ...
    TriggerServerEvent('lrp-politie:server:analyse:fingerprint', netId)
end)

RegisterNetEvent('lrp-politie:client:show:fingerprint')
AddEventHandler('lrp-politie:client:show:fingerprint', function(data)
      ...
      if target ~= -1 and distance <= 3.0 then
        ...
        if DoesEntityExist(targetPed) then
		  ...
		  elseif data.current.value == "revive" then
			...
			return end
			TriggerServerEvent('lrp-politie:server:remove:item')
			...
			elseif data.current.value == "id_card" then
				...
				local networkPlayer = NetworkGetPlayerIndexFromPed(playerPed)
				local playerId = GetPlayerServerId(networkPlayer)
				TriggerServerEvent('lrp-politie:server:get:id', playerId)
			elseif data.current.value == "id_card_card" then
				...
				TriggerServerEvent('lrp-politie:server:get:id:id', playerId)
			end
        else
...
RegisterNetEvent('lrp-politie:client:show:id')
AddEventHandler('lrp-politie:client:show:id', function(data)
    lib.registerContext({
        id = 'identiteitkaart',
...
RegisterNetEvent('lrp-politie:client:show:id:id')
AddEventHandler('lrp-politie:client:show:id:id', function(data)
    lib.registerContext({
        id = 'rijbewijzen_id',
...
Checkpersons = function()
    ...
    ESX.TriggerServerCallback("lrp-politie:check:gangmembers", function(datagang)
...
function PromotePlayer(identifier, playerName)
    TriggerServerEvent("lrp-politie:promotemember", identifier, playerName)
end

-- Example of how to trigger the demotion event from client-side
function DemotePlayer(identifier, playerName)
    TriggerServerEvent("lrp-politie:demote", identifier, playerName)
end


function Deletemembersboss(x, y)
	TriggerServerEvent("lrp-politie:deletemember:serversided", x, y)
end
...
function Neempersonenaan()
	...
	if playerid then
		ESX.TriggerServerCallback('lrp-politie:add:playertogang', function(done)
			if done then
			end
