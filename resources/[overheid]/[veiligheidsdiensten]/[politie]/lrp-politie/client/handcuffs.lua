...
RegisterNetEvent('jtm-development:police:client:set:player:vehicle')
AddEventHandler('jtm-development:police:client:set:player:vehicle', function(data)
	if not targetVehPed then
		...
		if distance < 3.0 then 
			TriggerServerEvent('jtm-development:police:server:set:player:vehicle', GetPlayerServerId(entityPlayer))
		else
...
RegisterNetEvent('jtm-development:police:client:cuff:player')
AddEventHandler('jtm-development:police:client:cuff:player', function(data)
    ...
    if distance < 2.0 and entityPlayer then 
        if count >= 1 then
            ...
            TriggerServerEvent('jtm-development:police:server:RemoveItem')
            
            if dist < 200.0 then 
                TriggerServerEvent('jtm-development:police:server:ProcessCuffs', 0, GetPlayerServerId(entityPlayer))
            else 
                TriggerServerEvent('jtm-development:police:server:ProcessCuffs', 1, GetPlayerServerId(entityPlayer))
            end
...
RegisterNetEvent('jtm-development:police:client:uncuff:player')
AddEventHandler('jtm-development:police:client:uncuff:player', function(data)
    local xPlayer = ESX.GetPlayerData()
    if xPlayer.job.name ~= 'police' then
        return
    end
    ...
    if distance < 2.0 and entityPlayer then 
        ...
        TriggerServerEvent('jtm-development:police:server:GiveItem')
        
        exports['vesx_audio']:PlayAudioDist(GetPlayerServerId(entityPlayer), 2.0, "handcuff", 0.3)
        TaskPlayAnim(PlayerPedId(), "mp_arresting", "a_uncuff", 8.0, -8, -1, 49, 0, false, false, false) 
        TriggerServerEvent('jtm-development:police:server:RemoveCuffs', 1, GetPlayerServerId(entityPlayer))
        Wait(2000)
        ClearPedTasks(PlayerPedId()) 
    else
        ESX.ShowNotification('error', 'Deze persoon is niet meer in de buurt!')
    end
end)



RegisterNetEvent('jtm-development:police:client:RemoveCuffs')
AddEventHandler('jtm-development:police:client:RemoveCuffs', function()
    if cuffed == true then 
	   cuffed = false
	   startAnim() 
	end
...
RegisterNetEvent('jtm-development:police:client:ProcessCuffs')
AddEventHandler('jtm-development:police:client:ProcessCuffs', function(x)
    ProcessCuffs(x)
