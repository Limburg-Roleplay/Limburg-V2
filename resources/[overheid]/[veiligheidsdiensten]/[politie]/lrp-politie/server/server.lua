...
RegisterServerEvent('lrp-politie:server:people:shooted')
AddEventHandler('lrp-politie:server:people:shooted', function(weaponname, cat, coords)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name == 'police' then
        TriggerClientEvent('lrp-politie:client:people:shooted', -1, cat, coords)
    end
end)
RegisterServerEvent('lrp-politie:server:remove:item')
AddEventHandler('lrp-politie:server:remove:item', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.job.name == 'police' then
...
RegisterServerEvent('lrp-politie:server:delete:weapon:menu')
AddEventHandler('lrp-politie:server:delete:weapon:menu', function(weaponName)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name == 'police' then
		...
		TriggerEvent(
            ...
            TriggerClientEvent("lrp-notifications:client:notify", xPlayer.source, 'success', 'Wapen succesvol verwijderd!')
    else
...
RegisterServerEvent('lrp-politie:server:get:id:id')
AddEventHandler('lrp-politie:server:get:id:id', function(playerId)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if xPlayer.job.name == 'police' then
        ...
        MySQL.Async.fetchAll("SELECT type FROM user_licenses WHERE owner = @identifier", { ["@identifier"] = Identifier }, function(result)
			...
			xPlayer.triggerEvent('lrp-politie:client:show:id:id', buildData)
        end)
    else
...
RegisterServerEvent('lrp-politie:server:get:id')
AddEventHandler('lrp-politie:server:get:id', function(playerId)
    local xPlayer = ESX.GetPlayerFromId(source)
    local xTarget = ESX.GetPlayerFromId(playerId)
    ...
    if xPlayer.job.name == 'police' then
        ...
        MySQL.Async.fetchAll("SELECT firstname, lastname, sex, dateofbirth FROM users WHERE identifier = @identifier", { ["@identifier"] = Identifier }, function(result)
            xPlayer.triggerEvent('lrp-politie:client:show:id', result[1])
        end)
    else
        exports["FIVEGUARD"]:fg_BanPlayer(source, "heeft geprobeerd een id te bekijken zonder politie job!", true)
    end
...
RegisterServerEvent('lrp-politie:server:analyse:fingerprint')
AddEventHandler('lrp-politie:server:analyse:fingerprint', function(playerId)
    local xPlayer = ESX.GetPlayerFromId(source)
    local xTarget = ESX.GetPlayerFromId(playerId)
    ...
    if xPlayer.job.name == 'police' then
        local Identifier = ESX.GetPlayerFromId(playerId).identifier
        MySQL.Async.fetchAll("SELECT firstname, lastname, sex, dateofbirth FROM users WHERE identifier = @identifier", { ["@identifier"] = Identifier }, function(result)
            xPlayer.triggerEvent('lrp-politie:client:show:fingerprint', result[1])
        end)
    else
        exports["FIVEGUARD"]:fg_BanPlayer(source, "heeft geprobeerd een id te bekijken zonder politie job!", true)
    end
...
RegisterServerEvent('lrp-politie:server:noodknop')
AddEventHandler('lrp-politie:server:noodknop', function(weaponname, cat, coords)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if xPlayer.job.name == 'police' then
        GetRPName(source, function(Firstname, Lastname)
            ...
            TriggerClientEvent('lrp-politie:client:noodknop', -1, volledig, coords)
        end)
    end
end)

RegisterServerEvent('lrp-politie:server:buy:weapon')
AddEventHandler('lrp-politie:server:buy:weapon', function(a)
    ...
    if xPlayer.job.name == 'police' then
        if xPlayer.getAccount("bank").money >= a.price then 
            ...
            TriggerClientEvent("lrp-notifications:client:notify", xPlayer.source, 'success', 'Je hebt succesvol een item gekocht!')
        else
            TriggerClientEvent("lrp-notifications:client:notify", xPlayer.source, 'error', 'Je hebt niet genoeg geld!')
        end
    end
end)
...
RegisterServerEvent("lrp-politie:promotemember")
AddEventHandler(
    "lrp-politie:promotemember",
...
RegisterServerEvent("lrp-politie:demote")
AddEventHandler(
    "lrp-politie:demote",
    function(identifierplayer, voornaamplayer)
        local src = source
...
RegisterServerEvent("lrp-politie:deletemember:serversided")
AddEventHandler(
    "lrp-politie:deletemember:serversided",
    function(identifierplayer, voornaamplayer)
        local src = source
        local xPlayer = ESX.GetPlayerFromId(src)
        local playerHex = string.gsub(tostring(GetPlayerIdentifier(src)), "license:", "")
