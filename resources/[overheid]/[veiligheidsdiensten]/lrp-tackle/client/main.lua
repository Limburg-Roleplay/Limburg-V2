...
local function checkJob(jobName)
	local found = false
	for k,v in pairs(authorizedJobs) do
		if v == jobName then
			canTackle = true
			return true
		end
...
RegisterCommand('tackle', function()
	if canTackle and IsControlPressed(0, 21) and not IsPlayerDead(PlayerId()) and not IsPedRagdoll(PlayerPedId()) and not IsPedFalling(PlayerPedId()) and not isTackling and GetGameTimer() - lastTackleTime > 10 * 1000 then
		local closestPlayer, distance = ESX.Game.GetClosestPlayer(false, true)
