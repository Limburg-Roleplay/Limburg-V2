...
RegisterServerEvent('lrp-ambulance:server:revive:player')
AddEventHandler('lrp-ambulance:server:revive:player', function(targetId)
    ...
    TriggerClientEvent('lrp-ambulance:client:staffrevive:player', xTarget.source)
end)

RegisterServerEvent('lrp-ambulance:startKnockoutTimer')
AddEventHandler('lrp-ambulance:startKnockoutTimer', function()
	local _src = source
	...
	Citizen.CreateThread(function()
		while true do
			...
			if GetGameTimer() - currentGameTime >= 30000 then
				...
				TriggerClientEvent('lrp-ambulance:stopKnockout', _src)
				break
			end
		end
	end)
end)
RegisterServerEvent('lrp-ambulance:server:heal:player')
AddEventHandler('lrp-ambulance:server:heal:player', function(targetId)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('lrp-ambulance:donecpr')
AddEventHandler('lrp-ambulance:donecpr', function(xTarget)
...
ESX.RegisterServerCallback('lrp-ambulance:server:callback:get:death:status', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	MySQL.Async.fetchScalar('SELECT is_dead FROM users WHERE identifier = @identifier', {
		['@identifier'] = xPlayer.identifier
	}, function(isDead)
...
ESX.RegisterCommand('reviveall', 'admin', function(xPlayer, args, showError)
	for _, playerId in ipairs(GetPlayers()) do
		if Player(playerId).state.dead then
			TriggerClientEvent('lrp-ambulance:client:staffrevive:player', playerId)
            exports['jtm-hungerthirst']:setPlayerStatus(xPlayer.source, playerId, 100, 100)
		end
	end
end, false)
...
AddEventHandler('txAdmin:events:healedPlayer', function(eventData)
	...
	if eventData.id == -1 then
		for _, playerId in ipairs(GetPlayers()) do
			if Player(playerId).state.dead then
				TriggerClientEvent('lrp-ambulance:client:staffrevive:player', playerId)
                exports['jtm-hungerthirst']:setPlayerStatus(source, playerId, 100, 100)
			end
		end
	else
		if Player(eventData.id).state.dead then
			TriggerClientEvent('lrp-ambulance:client:staffrevive:player', eventData.id)
            exports['jtm-hungerthirst']:setPlayerStatus(source, eventData.id, 100, 100)
		end
	end
end)

ESX.RegisterCommand('revive', {'admin', 'mod', 'staff'}, function(xPlayer, args, showError)
    if not exports["lrp-staffdienst"]:inDienst(xPlayer.source) then
        return
            TriggerClientEvent("lrp-notifications:client:notify", xPlayer.source, 'error',
                'Je bent niet in dienst, zorg dat je <br> /staffdienst hebt gedaan')
    end
	args.playerId.triggerEvent('lrp-ambulance:client:staffrevive:player')
    exports['jtm-hungerthirst']:setPlayerStatus(xPlayer.source, args.playerId.source, 100, 100)
end, true, {
...
RegisterServerEvent('lrp-ambulance:server:getGear')
AddEventHandler('lrp-ambulance:server:getGear', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.job.name == 'ambulance' then
		if xPlayer.getMoney() > 500 then
			...
			TriggerClientEvent('lrp-notifications:client:notify', source, 'error', 'Je hebt hier niet genoeg geld voor!', 5000)
		end
	else
...
RegisterServerEvent('lrp-ambulance:removeItemsOnDeath')
AddEventHandler('lrp-ambulance:removeItemsOnDeath', function()
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback(
    "lrp-ambulance:check:gangmembers",
...
RegisterServerEvent("lrp-ambulance:promotemember")
AddEventHandler(
    "lrp-ambulance:promotemember",
...
RegisterServerEvent("lrp-ambulance:demote")
AddEventHandler(
    "lrp-ambulance:demote",
    function(identifierplayer, voornaamplayer)
        local src = source
        local xPlayer = ESX.GetPlayerFromId(src)
        local playerHex = string.gsub(tostring(GetPlayerIdentifier(src)), "license:", "")
...
RegisterServerEvent("lrp-ambulance:deletemember:serversided")
AddEventHandler(
    "lrp-ambulance:deletemember:serversided",
    function(identifierplayer, voornaamplayer)
        local src = source
        local xPlayer = ESX.GetPlayerFromId(src)
        local playerHex = string.gsub(tostring(GetPlayerIdentifier(src)), "license:", "")
...
ESX.RegisterServerCallback(
    "lrp-ambulance:add:playertogang",
