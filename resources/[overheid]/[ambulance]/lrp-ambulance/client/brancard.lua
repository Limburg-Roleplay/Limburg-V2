    ...
    options = {
        {
            ...
            event = 'lrp-ambulance:client:remove:brancard',
            job = 'ambulance'
        },
		{
            icon = "fa-solid fa-truck-medical",
            label = "Pak brancard vast",
            event = 'lrp-ambulance:pick:up:brancard',
            job = 'ambulance'
        },
		{
            icon = "fa-solid fa-bed-pulse",
            label = "Liggen op brancard",
            event = 'lrp-ambulance:client:lay:brancard',
        },
    },
    distance = 2.5
})
	...
	options = {
		{
			...
			action = function(vehicle)
				if not Entity(vehicle).state.brancard then
					...
					TriggerEvent('lrp-ambulance:pick:up:brancard', {['entity'] = wheelchair})
				else
					local brancard = Entity(vehicle).state.brancard
					TriggerEvent('lrp-ambulance:pick:up:brancard', {['entity'] = brancard})
					TriggerServerEvent('lrp-ambulance:server:set:vehicle:busy:brancard', NetworkGetNetworkIdFromEntity(vehicle), false, false)
				end
			end,
		...
		},
		{
			...
			action = function(vehicle)
				...
				TriggerServerEvent('lrp-ambulance:server:set:vehicle:busy:brancard', NetworkGetNetworkIdFromEntity(vehicle), IsAttached, true)
				TriggerServerEvent('lrp-ambulance:server:no:busy:brancard:pickup', NetworkGetNetworkIdFromEntity(IsAttached))
				IsAttached = false
				Wait(200)
...
RegisterNetEvent('lrp-ambulance:pick:up:brancard')
AddEventHandler('lrp-ambulance:pick:up:brancard', function(data)
	local entity = data.entity
	local PlayerPed = PlayerPedId()
    local PlayerPos = GetEntityCoords(PlayerPed)

	ESX.TriggerServerCallback('lrp-ambulance:server:is:brancard:busy:pickup', function(bool)
		if not bool then
			...
			Citizen.CreateThread(function()
				while IsAttached do
					...
					if IsControlJustPressed(0, 38) then
						TriggerServerEvent('lrp-ambulance:server:no:busy:brancard:pickup', NetworkGetNetworkIdFromEntity(entity))
						IsAttached = false
...
RegisterNetEvent('lrp-ambulance:client:lay:brancard')
AddEventHandler('lrp-ambulance:client:lay:brancard', function(data)
	local entity = data.entity
	...
	ESX.TriggerServerCallback('lrp-ambulance:server:is:brancard:busy', function(bool)
		if not bool then
			LoadAnim(inBedDicts)
			...
			while IsLayingOnBed do
				...
				if IsControlJustPressed(0, 38) then
					TriggerServerEvent('lrp-ambulance:server:no:busy:brancard', NetworkGetNetworkIdFromEntity(entity))
					ClearPedTasks(PlayerPedId())
					IsLayingOnBed = false
					exports[''..Config.interaction..'']:clearInteraction()
					break
