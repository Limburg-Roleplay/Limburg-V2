...
local function LeaveBed()
    ...
    TriggerServerEvent('lrp-ambulance:server:LeaveBed', bedOccupying)
    FreezeEntityPosition(bedObject, true)
...
RegisterNetEvent('lrp-ambulance:client:SendToBed', function(id, data, isRevive)
    bedOccupying = id
...
RegisterNetEvent('lrp-ambulance:client:SetBed', function(id, isTaken)
    Config.CheckIn.beds[id].taken = isTaken
end)

RegisterNetEvent('lrp-ambulance:client:SetBed2', function(id, isTaken)
...
RegisterNetEvent('qb-ambulancejob:checkin', function()
    ...
    exports['lrp-progressbar']:Progress({
        ...
        if not cancelled then
            ...
            if bedId then
                TriggerServerEvent("lrp-ambulance:server:sendToBed", bedId, true)
            else
                exports['lrp-notifications']:Notify('error', 'Alle bedden zijn bezet!', 5000)
            end
        end
    end)
end)
...
RegisterNetEvent('qb-ambulancejob:beds', function()
    if GetAvailableBed(closestBed) then
        TriggerServerEvent("lrp-ambulance:server:sendToBed", closestBed, true)
    else
        exports['lrp-notifications']:Notify('error', 'Alle bedden zijn bezet!', 5000)
    end
end)
...
CreateThread(function()
    ...
    exports.ox_target:addModel(Config.CheckIn.researchtable, {
        {
            name = 'lrp-ambulance:researchtable',
