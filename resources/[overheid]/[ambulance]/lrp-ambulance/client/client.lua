...
RegisterNetEvent('lrp-ambulance:client:remove:brancard')
AddEventHandler('lrp-ambulance:client:remove:brancard', function(data)
    local entity = data.entity

    TriggerServerEvent('lrp-ambulance:server:remove:brancard', NetworkGetNetworkIdFromEntity(entity))
end)
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    ...
    ESX.TriggerServerCallback('wasabi_ambulance:checkDeath', function(dead)
        if dead then
            ...
            exports['lrp-ambulance']:combatlog()
        end
    end)
...
RegisterNetEvent('lrp-ambulance:client:send:factuur')
AddEventHandler('lrp-ambulance:client:send:factuur', function(data)
	local entity = data.entity
	...
	if distance < 2.0 then
		local options = {
			{
				...
				['event'] = 'lrp-ambulance:client:search:fine:list',
				['args'] = { entity = entity }
			}
		}
		...
		for k,v in pairs(Config.Fines) do
			options[#options+1] = {
				title = k,
				event = 'lrp-ambulance:client:give:fine:list',
...
RegisterNetEvent('lrp-ambulance:client:search:fine:list')
AddEventHandler('lrp-ambulance:client:search:fine:list', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
		...
		if input then 
			...
			for i=1, #found do
				options[#options+1] = {
					...
					event = 'lrp-ambulance:client:give:fine:finalise',
...
function KnockOutPlayerPed()
	...
	TriggerServerEvent("lrp-ambulance:startKnockoutTimer")
	AnimpostfxPlay('DeathFailOut', 0, false)
	-- ESX.ShowNotification('error', "Je bent bewusteloos.")	
    lib.notify({
        title = 'Knockout',
...
RegisterNetEvent("lrp-ambulance:stopKnockout", function()
	isKnockoutThreadRunning = false
end)
...
RegisterNetEvent('lrp-ambulance:client:give:fine:list')
AddEventHandler('lrp-ambulance:client:give:fine:list', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
	if distance < 2.0 then
		local options = {}
...
RegisterNetEvent('lrp-ambulance:client:give:fine:finalise')
AddEventHandler('lrp-ambulance:client:give:fine:finalise', function(data)
...
AddEventHandler('esx_ambulancejob:revive', function()
    TriggerEvent("lrp-ambulance:client:staffrevive:player")
end)
...
Checkpersons = function()
    ...
    ESX.TriggerServerCallback("lrp-ambulance:check:gangmembers", function(datagang)
...
function PromotePlayer(identifier, playerName)
    TriggerServerEvent("lrp-ambulance:promotemember", identifier, playerName)
end

-- Example of how to trigger the demotion event from client-side
function DemotePlayer(identifier, playerName)
    TriggerServerEvent("lrp-ambulance:demote", identifier, playerName)
end


function Deletemembersboss(x, y)
	TriggerServerEvent("lrp-ambulance:deletemember:serversided", x, y)
end
...
function Neempersonenaan()
	...
	if playerid then
		ESX.TriggerServerCallback('lrp-ambulance:add:playertogang', function(done)
			if done then
			end
