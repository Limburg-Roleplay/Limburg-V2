...
exports.qtarget:Player({
	options = {
		{
			event = "lrp-ambulance:client:ped-inter:cpr",
		...
		{
			event = 'lrp-cuff:client:dragging:ambulance',
      ...
      if target ~= -1 and distance <= 3.0 then
        ...
        if DoesEntityExist(targetPed) then
          if data.current.value == "verzorgen" then
            TriggerEvent('lrp-ambulance:client:ped-inter:verzorgen', { entity = targetPed })
          elseif data.current.value == "omhooghelpen" then
            TriggerEvent('lrp-ambulance:client:ped-inter:revive', { entity = targetPed })
...
RegisterNetEvent('lrp-ambulance:client:ped-inter:verzorgen', function(data)
    ...
    if distance < 2.0 then
        if IsPedDeadOrDying(entityPlayer, 1) then
            ...
            ESX.TriggerServerCallback('lrp-ambulance:server:check:zorgverzekering', function(bool)
                if bool then
                    exports['lrp-notifications']:notify('success',
                        'Je hebt de inwoner een pleister gegeven!<br>Deze kosten vallen onder de zorgverzekering van de inwoner.',
                        5000)
                    TriggerServerEvent('lrp-ambulance:server:heal:player', GetPlayerServerId(entityPlayer))
                else
                    exports['lrp-notifications']:notify('success',
            ...
            exports['lrp-notifications']:notify('error', 'Deze inwoner heeft geen pleisster nodig!', 5000)
        end
    else
        exports['lrp-notifications']:notify('error', 'Er is niemand in de buurt!', 5000)
    end
end)

RegisterNetEvent('lrp-ambulance:client:ped-inter:cpr', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
	local entityPlayer = ESX.Game.GetPlayerFromPed(entity)
	local xTarget = GetPlayerServerId(entityPlayer)
		...
		if IsPedDeadOrDying(entityPlayer, 1) then
			if isBusy == false then
				...
				for i = 1, 15 do
					...
					if i == 15 then
						TriggerServerEvent('lrp-ambulance:donecpr', xTarget)
					end
				end
			else
				exports['lrp-notifications']:notify('error', 'Je bent al met iets bezig!', 5000)
			end
		else
			exports['lrp-notifications']:notify('error', 'Deze inwoner heeft geen hulp nodig!', 5000)
		end
		isBusy = false
	else
		exports['lrp-notifications']:notify('error', 'Er is niemand in de buurt!', 5000)
...
RegisterNetEvent('lrp-ambulance:client:ped-inter:revive', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
	local entityPlayer = ESX.Game.GetPlayerFromPed(entity)
	local xTarget = GetPlayerServerId(entityPlayer)
	...
	if distance < 2.0 then
        if DoesEntityExist(entity) then
            ...
            if health == 0 then
                ...
                TriggerServerEvent('lrp-ambulance:server:revive:player', GetPlayerServerId(entityPlayer))
                exports['lrp-notifications']:notify('success', 'Je hebt de inwoner omhoog geholpen.', 5000)
