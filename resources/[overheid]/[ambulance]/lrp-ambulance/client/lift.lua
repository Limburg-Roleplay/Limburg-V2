...
OpenLiftMenu = function()
    ...
    for i=1, #configOptions do
        ...
        if lift then
            options[#options+1] = {
                ...
                event = 'lrp-ambulance:client:teleport:player',
                args = { index = i }
            }
        end
    end
    ...
    lib.registerContext({
        id = 'lrp-ambulance:lift:menu',
        title = 'Ambulance Lift',
        options = options
    })
    lib.showContext('lrp-ambulance:lift:menu')
end

RegisterNetEvent('lrp-ambulance:client:teleport:player', function(data)
