...
Citizen.CreateThread(function()
	exports['qtarget']:Player({
		options = {
			{
				event = 'lrp-ambulance:client:send:factuur',
...
OpenVehiclesMenu = function(type, configType, data)
	...
	for i = 1, #configType[data.category].vehicles do
		vehicles[#vehicles + 1] = {
			...
			event = 'lrp-ambulance:client:spawn:vehicle',
...
GetGear = function()
	TriggerServerEvent('lrp-ambulance:server:getGear', false)
end
...
RegisterNetEvent('lrp-ambulance:client:own:cloakroom')
AddEventHandler('lrp-ambulance:client:own:cloakroom', function()
	if ESX.PlayerData.job.name == 'ambulance' then
		exports['' .. Config.Kleding .. '']:openSavedOutfits()
	end
end)

RegisterNetEvent('lrp-ambulance:client:ambu:cloakroom')
AddEventHandler('lrp-ambulance:client:ambu:cloakroom', function()
	if ESX.PlayerData.job.name == 'ambulance' then
	...
	if ESX.PlayerData.job.name == 'ambulance' then
		local options = {
			[1] = {
				...
				['event'] = 'lrp-ambulance:client:own:cloakroom'
			},
			[2] = {
				['title'] = 'Algemene kleedkamer',
				['description'] = 'Bekijk de Ambulance outfits',
				['event'] = 'lrp-ambulance:client:ambu:cloakroom'
			},
		}

		lib.registerContext({
			id = 'lrp-ambulance:check-cloakroom',
			title = 'Ambulance: Bekijk opties van Kleedkamer',
			options = options
		})

		lib.showContext('lrp-ambulance:check-cloakroom')
	end
end

RegisterNetEvent('lrp-ambulance:client:spawn:vehicle')
AddEventHandler('lrp-ambulance:client:spawn:vehicle', function(data)
...
respawnPed = function(ped, coords, heading)
	...
	exports['lrp-ambulance']:respawncombatlog()
end
...
function respawnPlayer()
    Citizen.CreateThread(function()
        ...
        if deathPosition and deathPosition ~= vector3(0, 0, 0) then
            ...
            exports['lrp-ambulance']:respawncombatlog()
            TriggerServerEvent('lrp-ambulance:removeItemsOnDeath')
...
RegisterNetEvent('lrp-ambulance:client:staffrevive:player')
AddEventHandler('lrp-ambulance:client:staffrevive:player', function()
	local ScriptName = GetInvokingResource()
	if ScriptName ~= nil then
...
RegisterNetEvent('lrp-ambulance:client:noodvoeding:player', function(targetId)
