...
OpenVehiclesMenu = function(type, configType, data)
	...
	for i=1, #configType[data.category].vehicles do 
		vehicles[#vehicles+1] = {
			...
			event = 'lrp-anwb:client:spawn:vehicle',
...
RegisterNetEvent('lrp-anwb:client:own:cloakroom')
AddEventHandler('lrp-anwb:client:own:cloakroom', function()
...
RegisterNetEvent('lrp-anwb:client:anwb:cloakroom')
AddEventHandler('lrp-anwb:client:anwb:cloakroom', function()
	if ESX.PlayerData.job.name == 'mechanic' then
	...
	if ESX.PlayerData.job.name == 'mechanic' then
		local options = {
			[1] = {
				...
				['event'] = 'lrp-anwb:client:own:cloakroom'
			},
			[2] = {
				['title'] = 'Algemene kleedkamer',
				['description'] = 'Bekijk de ANWB outfits',
				['event'] = 'lrp-anwb:client:anwb:cloakroom'
			},
		}
...
RegisterNetEvent('lrp-anwb:client:spawn:vehicle')
AddEventHandler('lrp-anwb:client:spawn:vehicle', function(data)
...
RegisterNetEvent('lrp-anwb:client:toggle:flatbed')
AddEventHandler('lrp-anwb:client:toggle:flatbed', function(data)
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
		...
		options = {
			{
				...
				event = 'lrp-resources:client:objDelete:pion',
				job = 'mechanic'
			},
		},
		distance = 2.5
	...
	exports['qtarget']:Vehicle({
		options = {
			{
				event = 'lrp-anwb:client:repair:vehicle',
				icon = 'fas fa-screwdriver-wrench',
				label = 'Voertuig repareren',
				item = 'repairkit'
			},
			{
				event = 'lrp-anwb:client:wash:vehicle',
				icon = 'fas fa-hand-sparkles',
				label = 'Voertuig schoonmaken',
				item = 'washand'
			},
			{
				event = 'lrp-anwb:client:toggle:flatbed',
	...
	exports['qtarget']:Player({
		options = {
			{
				event = 'lrp-anwb:client:send:factuur',
...
RegisterNetEvent('lrp-anwb:client:set:clothing')
AddEventHandler('lrp-anwb:client:set:clothing', function(data)
...
RegisterNetEvent('lrp-anwb:client:repair:vehicle')
AddEventHandler('lrp-anwb:client:repair:vehicle', function(data)
	...
	if vehicle ~= nil then 
		...
		TriggerServerEvent('lrp-anwb:server:repair:vehicle')
		...
		ESX.ShowNotification('error', 'Dit voertuig bestaat niet')
	end
end)

RegisterNetEvent('lrp-anwb:client:wash:vehicle')
AddEventHandler('lrp-anwb:client:wash:vehicle', function(data)
	local vehicle = data.entity
	if vehicle ~= nil then 
		...
		TriggerServerEvent('lrp-anwb:server:wash:vehicle')
		exports[''..Config.Progress..'']:Progress({
...
RegisterNetEvent('lrp-anwb:client:send:factuur')
AddEventHandler('lrp-anwb:client:send:factuur', function(data)
	...
	if distance < 2.0 then
		local options = {
			{
				...
				['event'] = 'lrp-anwb:client:search:fine:list',
				['args'] = { entity = entity }
			}
		}
		...
		for k,v in pairs(Config.Fines) do
			options[#options+1] = {
				title = k,
				event = 'lrp-anwb:client:give:fine:list',
...
RegisterNetEvent('lrp-anwb:client:search:fine:list')
AddEventHandler('lrp-anwb:client:search:fine:list', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
		...
		if input then 
			...
			for i=1, #found do
				options[#options+1] = {
					...
					event = 'lrp-anwb:client:give:fine:finalise',
...
RegisterNetEvent('lrp-anwb:client:give:fine:list')
AddEventHandler('lrp-anwb:client:give:fine:list', function(data)
	local entity = data.entity
	local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(entity))
	if distance < 2.0 then
		local options = {}
...
RegisterNetEvent('lrp-anwb:client:give:fine:finalise')
AddEventHandler('lrp-anwb:client:give:fine:finalise', function(data)
...
Checkpersons = function()
    ...
    ESX.TriggerServerCallback("lrp-anwb:check:gangmembers", function(datagang)
...
function PromotePlayer(identifier, playerName)
    TriggerServerEvent("lrp-anwb:promotemember", identifier, playerName)
end

-- Example of how to trigger the demotion event from client-side
function DemotePlayer(identifier, playerName)
    TriggerServerEvent("lrp-anwb:demote", identifier, playerName)
end


function Deletemembersboss(x, y)
	TriggerServerEvent("lrp-anwb:deletemember:serversided", x, y)
end
...
function Neempersonenaan()
	...
	if playerid then
		ESX.TriggerServerCallback('lrp-anwb:add:playertogang', function(done)
			if done then
			end
