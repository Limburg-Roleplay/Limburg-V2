...
-- functions
LRP.OpenManagementMenu = function(job)
...
exports('OpenManagementMenu', LRP.OpenManagementMenu)
				...
				local function ChangeEmployee()
					ESX.TriggerServerCallback('vesx_jobsmenu:server:manageEmployee', function(success, task, err)
						if success then
							...
							ESX.ShowNotification('error', err, 5000)
    ...
    lib.registerMenu({
        ...
        if args ~= nil then
			...
			elseif args.action == 'search' then
				LRP.SearchPlayerDialogue(job, players)
			else
				ESX.TriggerServerCallback('vesx_jobsmenu:server:manageEmployee', function(success, task, err)
					if success then
						if task == 'updated_grade' then
...
LRP.SearchPlayerDialogue = function(job, fullPlayers)
	...
	if input ~= nil then
		...
		if not found then
			ESX.ShowNotification('error', TranslateCap('cl_niks'), 5000)
			LRP.SearchPlayerDialogue(job, fullPlayers)
		end
	else
		HireEmployeeManagment(job)
	end
...
LRP.OpenOutfitCategoryMenu = function(jobOutfits)
	if jobOutfits ~= nil then
		...
		lib.registerContext({
			id = 'lrp:jobmenu:clothingCategory',
			title = 'Outfitmenu',
			options = categories
		})

		lib.showContext('lrp:jobmenu:clothingCategory')
	end
end
exports('OpenOutfitMenu', LRP.OpenOutfitCategoryMenu)


LRP.OpenOutfitMenu = function(data)
	local outfits = {}
	...
	lib.registerContext({
		id = 'lrp:jobmenu:clothing',
		title = data.category,
		options = outfits
	})

	lib.showContext('lrp:jobmenu:clothing')
end
...
LRP.ToggleDuty = function(job)
	if cooldown <= 0 then
		cooldown = 5
		TriggerServerEvent('lrp-jobsmenu:server:toggleDuty', job)
	else
		ESX.ShowNotification('error', 'Wacht nog ' .. cooldown .. ' seconden met in-/uitklokken.')
	end
end

exports('ToggleDuty', LRP.ToggleDuty)