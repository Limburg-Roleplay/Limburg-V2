...
ESX.RegisterServerCallback('exios-boatshop:server:cb:get:shared', function(source, cb)
    cb(Shared)
end)
ESX.RegisterServerCallback('frp-boatdealer:isPlateTaken', function (source, cb, plate)
...
ESX.RegisterServerCallback('exios-boatshop:server:cb:quickSell', function(source, cb, plate, props)
...
ESX.RegisterServerCallback('exios-boatshop:server:cb:buyVehicle', function(source, cb, class, id, colour)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('exios-boatshop:server:saveVehicle', function(source, cb, plate, props, model)
    local xPlayer = ESX.GetPlayerFromId(source)

    exports['oxmysql']:query('INSERT INTO `owned_vehicles` (owner, plate, vehicle, stored, type) VALUES (?, ?, ?, ?, ?)', { xPlayer.identifier, plate, json.encode(props), 0, 'boat' })
...
RegisterNetEvent('exios-boatshop:server:handler:requestscreenshot')
AddEventHandler('exios-boatshop:server:handler:requestscreenshot', function(vehName, cameraId, vehBrand, label)
    local src = source

    exports['screenshot-basic']:requestClientScreenshot(src, {
        fileName =  'resources/[frp]/frp-boatdealer/src/html/voertuigen/' .. vehBrand .. '_' .. vehName .. '_' .. cameraId .. '_' .. label .. '.png'
    }, function(err, data)
        print(data)
    end)
end)
...
ESX.RegisterCommand('boatdealer_fotos', 'admin', function(xPlayer, args, showError)
    xPlayer.triggerEvent('exios-boatshop:client:handler:startFotobook', xPlayer.getGroup())
