...
CreateThread(function()
    ...
    ESX.TriggerServerCallback('exios-eilandshop:server:cb:get:shared', function(data)
        Shared = data
    end)
...
RegisterNetEvent('exios-eilandshop:client:open:catalogus')
AddEventHandler('exios-eilandshop:client:open:catalogus', function()
    local vehicles = {}
...
RegisterNUICallback('buyVehicle', function(data, cb)
    ...
    ESX.TriggerServerCallback('exios-eilandshop:server:cb:buyVehicle', function(buyCallback, plateText)
        ...
        for i=1, #Shared.SpawnPlaces do
            ...
            ESX.Game.SpawnVehicle(vehicle.model, Shared.SpawnPlaces[spawnPlace].coords, Shared.SpawnPlaces[spawnPlace].heading, function(veh)
                ...
                ESX.TriggerServerCallback('exios-eilandshop:server:saveVehicle', function(isDone)
...
Exios.Functions.ConfirmQuickSellMenu = function(vehicle, vehLabel, sellPrice)
    ...
    lib.registerContext({
        id = 'exios-eilandshop:client:confirm:quickSell',
    ...
    lib.showContext('exios-eilandshop:client:confirm:quickSell')
end
    ...
    for k,v in pairs(Shared.Vehicles) do 
        ...
        for i=1, #v do 
            ...
            if GetHashKey(model) == currentVehicle then 
                ESX.TriggerServerCallback('exios-eilandshop:server:cb:quickSell', function(bool)
                    if bool then 
...
Exios.Functions.SpawnShowroomBlips = function()
    for i=1, #Shared.Locations do 
        if Shared.Locations[i]['blip'] then 
            ...
            AddTextComponentSubstringPlayerName('Eilanddealer')
...
Exios.Functions.SpawnPeds = function()
    ...
    for i=1, #Shared.Locations do 
        ...
        exports.ox_target:addLocalEntity(Shared.Locations[i]['ped'], {
            {
                event = 'exios-eilandshop:client:open:catalogus',
