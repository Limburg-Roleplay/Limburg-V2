...
RegisterNetEvent('ox_appearance:wardrobe', function()
    ...
    for k, v in pairs(outfitNames) do
		...
		if tonumber(v) then
			outfitNames[k] = nil
			outfits[k] = nil
			TriggerServerEvent('ox_appearance:deleteOutfit', k, outfitNames)
		end
    end
...
AddEventHandler('ox_appearance:deleteOutfit', function(data)
	if data then
		...
		TriggerServerEvent('ox_appearance:deleteOutfit', data.slot, outfitNames)
	end
end)
...
AddEventHandler('ox_appearance:saveOutfit', function(data)
	...
	if data.slot == 'new' then
		data.slot = #outfitNames + 1
		local name = lib.inputDialog(locale('new_outfit'), {locale('outfit_name')})
		if tonumber(name[1]) then ESX.ShowNotification('error', 'Je kan nummers NIET als naam gebruiken.<br> Bijv. A1 kan wel.', 5000)  return end
...
exports('showOutfitMenu', function()
	lib.showContext('save_change')
end)
