...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		lib.notify({description = _U('create_a_character'), type = 'inform'})
	end
end)
	...
	for theData, value in pairs(myIdentity) do
		...
		elseif theData == "dateofbirth" then
			if value == "invalid" then
				reason = "Ongeldige geboortedatum!"
				break
			end
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Je spelersnaam is te kort of te lang.'
	end
	
	-- Don't allow special characters (doesn't always work)
	local count = 0
	for i in name:gmatch('[abcdefghijklmnopqrstuvwxyzåäöáàëéèüúùABCDEFGHIJKLMNOPQRSTUVWXYZÅÄÖÁÀËÉÈÜÚÙ0123456789 -]') do
		count = count + 1
	end
	if count ~= nameLength then
		return 'Je spelersnaam bevat speciale tekens die niet zijn toegestaan op deze server.'
	end
	...
	if spacesInName > 2 then
		return 'Uw naam bevat meer dan twee spaties'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'je naam moet beginnen met een hoofdletter.'
	end
