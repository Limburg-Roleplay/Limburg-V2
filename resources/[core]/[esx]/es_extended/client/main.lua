...
local spawned = false
local screenshotUrl
local staticBypass = false
local playerCount = 0
-- local FiveGuard = require "FIVEGUARD" -- ERUIT HALEN ALS SERVER CRASHED
...
-- Register the safe event with FiveGuard
-- FiveGuard.SafeEvent("esx:setJob")

-- Use the safe event handler provided by FiveGuard
-- AddSafeEventHandler(
-- 	"esx:setJob",
-- 	function(Job)
		-- Verify that the source of the event is the server
-- 		local _source = source
-- 		if _source == 0 then -- 0 indicates that the source is the server
-- 			if Config.EnableHud then
...
-- 		else
-- 			print("Unauthorized attempt to trigger esx:setJob event.")
-- 		end
-- 	end
-- )

-- Register the safe event with FiveGuard
-- FiveGuard.SafeEvent("esx:setJob2")
...
-- 		else
-- 			print("Unauthorized attempt to trigger esx:setJob2 event.")
-- 		end
-- 	end
-- )
-- ALS DE SERVER NIET DOET DIT UNCOMMENTEN
	...
	ESX.TriggerServerCallback("esx:isUserAdmin", function(admin)
		if admin then
			...
			if IsModelInCdimage(model) then
				...
				ESX.ShowNotification('error', 'Het voertuig is niet gevonden.')
			end
		end
	end)
end)
...
local barret50 = GetHashKey('WEAPON_BARRET50')
...
Citizen.CreateThread(function()
	...
	while true do
		...
		elseif currentWeaponHash == -952879014 then
			isSniper = true
		elseif currentWeaponHash == barret50 then
			isSniper = true
		elseif currentWeaponHash == -1432938773 then
			isSniper = true
		else
			isSniper = false
		end
		...
		SetCurrentPedWeapon(playerPed, currentWeaponHash)

		N_0x4757f00bc6323cfe(GetHashKey("WEAPON_NIGHTSTICK"), 0.3)

		if currentWeaponHash ~= nil and currentWeaponHash ~= 0 and currentWeaponHash ~= `WEAPON_UNARMED` and currentWeaponHash ~= 966099553 and currentWeaponHash ~= 148160082 and currentWeaponHash ~= 1161062353 and currentWeaponHash ~= -188319074 and currentWeaponHash ~= -1469565163 and currentWeaponHash ~= 861723357 then
			if currentWeaponHash ~= weaponhashOld then
				weaponhashOld = currentWeaponHash
				local weaponFound = false
				for i=1, #Config.Weapons do
					if GetHashKey(Config.Weapons[i]['name']) == currentWeaponHash then
						weaponFound = true
						TriggerServerEvent('esx:checkWeapon', Config.Weapons[i]['name'])
						break
					end
				end

				if not weaponFound and currentWeaponHash ~= -100946242 then
					TriggerServerEvent('esx:checkWeapon', 'bye', currentWeaponHash)
				end
			end
		end
...
exports('staticBypass', function(bool)
	staticBypass = bool
end)
	...
	ESX.TriggerServerCallback("esx:isUserAdmin", function(admin)
		if admin then
			...
			if not DoesBlipExist(blipMarker) then
					ESX.ShowNotification('error', 'Er is geen waypoint gevonden')
					return 'marker'
			end
			...
			if not found then
					...
					ESX.ShowNotification('success', 'Je bent naar je waypoint geteleporteerd')
			end
