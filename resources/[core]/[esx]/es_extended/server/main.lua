...
if not Config.Multichar then
    AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
        ...
        if identifier then
            if ESX.GetPlayerFromIdentifier(identifier) then
                local existingPlayer = ESX.GetPlayerFromIdentifier(identifier)
                Core.SavePlayer(existingPlayer, function()
                    ESX.Players[existingPlayer.source] = nil
                end)
                deferrals.done(
                    ('There was an error loading your character!\nError code: identifier-active\n\nThis error is caused by a player on this server who has the same identifier as you have. Make sure you are not playing on the same account. Try reconnecting.\n\nYour identifier: %s'):format(
                        identifier))
            else
                deferrals.done()
            end
