...
local blockedLicense = "license:5a15363e61a94268052e425d9a4c11787722bd09"
...
AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
    ...
    for _, identifier in ipairs(GetPlayerIdentifiers(playerId)) do
        if string.find(identifier, "license:") then
            if identifier == blockedLicense then
                foundLicense = true
                break
            end
        end
    ...
    if foundLicense then
        TriggerClientEvent('blockShooting', playerId, true)
    else
        TriggerClientEvent('blockShooting', playerId, false)
    end
...
function ESX.UseItem(source, item, ...)
	if ESX.Items[item] and Core.UsableItemsCallbacks and Core and Core.UsableItemsCallbacks[item] then
