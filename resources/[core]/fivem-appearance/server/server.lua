...
AddEventHandler("fivem-appearance:deleteOutfit", function(id)
	...
	MySQL.Async.fetchScalar('SELECT `name` FROM `outfits` WHERE `id` = @id', {
		['@id'] = id
	}, function(outfitName)
		if outfitName then
			MySQL.Async.execute('DELETE FROM `outfits` WHERE `id` = @id', {
				...
				local data = {
					title = 'Success',
					description = "Outfit '" .. outfitName .. "' is verwijderd.",
					type = 'success'
				}
				...
				if rowsChanged > 0 then
					...
					data.title = 'Error'
					data.description = "Er is een fout opgetreden bij het verwijderen van het outfit."
					data.type = 'error'
					TriggerClientEvent('ox_lib:notify', source, data)
				end
			end)
			...
			local data = {
				title = 'Error',
				description = "Outfit niet gevonden.",
				type = 'error'
			}
			TriggerClientEvent('ox_lib:notify', source, data)
		end
