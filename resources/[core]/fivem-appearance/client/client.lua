...
RegisterNetEvent('fivem-appearance:saveOutfit', function()
    ...
    if input then
        ...
        if name == nil or name == "" then
            lib.notify({title = "Fout", description = "Je moet iets van een naam invoeren", type = "error"})
            return
        end
        ...
        lib.notify({title = "Succes", description = "Je hebt de outfit met de naam '" .. name .. "' succesvol opgeslagen!", type = "success"})
    end
end)
    ...
    if outfits then
        ...
        for i=1, #outfits do
            Options[#Options + 1] = {
                ...
                event = 'fivem-appearance:confirmDeleteOutfit',
                args = outfits[i].id 
            }
        end
    else
...
RegisterNetEvent('fivem-appearance:confirmDeleteOutfit', function(outfitId)
    local outfits = lib.callback.await('fivem-appearance:getOutfits', 100)
