...
AddEventHandler('onMySQLReady', function()
  local result = MySQL.Sync.fetchAll('SELECT * FROM kofferbak')
  local data = nil
  if #result ~= 0 then
    for i=1,#result,1 do
...
function loadInvent(plate)
  local result = MySQL.Sync.fetchAll('SELECT * FROM kofferbak WHERE plate = @plate',
  {
    ['@plate'] = plate,
  })
  local data = nil
...
function MakeDataStore(plate)
  ...
  local dataStore   = CreateDataStore(plate, owned, data)
  SharedDataStores[plate] = dataStore
  MySQL.Async.execute('INSERT INTO kofferbak (plate,data,owned) VALUES (@plate,\'{}\',@owned)',
  {
    ['@plate'] = plate,
    ['@owned'] = owned,
  }
