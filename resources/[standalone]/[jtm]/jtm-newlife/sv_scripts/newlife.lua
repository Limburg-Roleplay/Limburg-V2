...
RegisterCommand("overheidnewlife", function(source, args, rawCommand)
    ...
    if xPlayer and (xPlayer.job.name == "police" or xPlayer.job.name == "kmar") then
        if not cooldowns[source] or (os.time() - cooldowns[source]) >= 30 then
            ...
            TriggerClientEvent('lrp-notifications:client:notify', source, "error", 'Je kunt dit commando niet zo snel herhalen. Wacht ' .. remainingTime .. ' seconden.', 4000)
        end
    end
end, false)
...
AddEventHandler("politiehbnewlife", function()
    ...
    if xPlayer and (xPlayer.job.name == "police" or xPlayer.job.name == "kmar") then
        if isPlayerDead(src) then
            TriggerClientEvent('lrp-notifications:client:notify', src, "info", 'Je zal over 2 minuten een nieuw leven krijgen!', 4000)
            Citizen.Wait(120000)
            if not isPlayerDead(src) then
                TriggerClientEvent('lrp-notifications:client:notify', src, "error", 'Je bent niet meer dood! Actie geannuleerd!', 4000)
            else
                TriggerClientEvent('hrp-troep:teleportPlayer', src, -347.2685, -362.7626, 31.5773)
                TriggerClientEvent('lrp-ambulance:client:staffrevive:player', src)
                TriggerClientEvent('lrp-notifications:client:notify', src, "success", 'Je hebt jezelf succesvol een nieuw leven gegeven!', 4000)
            end
        else
            TriggerClientEvent('lrp-notifications:client:notify', src, "error", 'Je bent niet dood!', 4000)
        end
    else
