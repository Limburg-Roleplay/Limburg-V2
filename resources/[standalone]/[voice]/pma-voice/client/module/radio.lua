...
function syncRadioData(radioTable, localPlyRadioName)
	...
	--ESX.TriggerServerCallback('pma-voice:getStreamerMode', function(data)
	--	if data then
	--		sendUIMessage({
	--			radioChannel = "###",
...
local fakeporto = false

RegisterKeyMapping('+fakeporto', 'Fakeporto', 'keyboard', 'CAPITAL')

RegisterCommand('+fakeporto', function()
    fakeporto = true
end, false)
RegisterCommand('-fakeporto', function()
    fakeporto = false
...
CreateThread(function()
    while true do
        ...
        if fakeporto then
			...
			while fakeporto == true do
				Citizen.Wait(1)
			end
			StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_enter", 8.0, 2.0, -1, 50, 2.0, 0, 0, 0)
