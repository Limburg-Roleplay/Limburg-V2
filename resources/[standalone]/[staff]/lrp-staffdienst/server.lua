...
RegisterCommand("staffdienst", function(source)
    ...
    if lastCommandTime[src] and os.time() - lastCommandTime[src] < 180 and not xPlayer.getGroup() == 'hogerop' and not xPlayer.getGroup() == 'owner' and not xPlayer.getGroup() == 'admin' then  
        TriggerClientEvent("okokNotify:Alert", src, 'Info', 'Je moet nog wachten voor je deze commando weer kan gebruiken.', 5000, 'error')
        return
    end
    ...
    local discordlog = exports["lrp-logging"]:createlog("https://discord.com/api/webhooks/1283468399741108234/l3hS07mQc484w5aCyuKlEjrF6fmzaZq8Me3T5xGZzyRvxWhAnemFZvbtQV4mrjAIFcwh", "Staffdienst - Uit Dienst")
    ...
    if StaffIndienst[src] then
        ...
        TriggerClientEvent("okokNotify:Alert", src, 'Info', 'Staff Dienst is succesvol uitgezet.', 5000, 'success')
        ...
        TriggerClientEvent("okokNotify:Alert", src, 'Info', 'Staff Dienst is succesvol aangezet.', 5000, 'success')
