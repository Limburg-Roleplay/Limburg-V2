...
AddEventHandler('jtm_randomcrate:StartDropCrate', function(id, realCrateId)
    crateID = id
            ...
            if dist < 200.0 then
                ...
                if dist < 3.0 and DoesEntityExist(CrateObject) and not isOpening then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ESX.TriggerServerCallback('jtm_randomcrate:checkOpeningState', function(canOpen)
                            if canOpen then
                                TriggerServerEvent('jtm_randomcrate:StartOpeningCrate', realCrateId)
                                isOpening = true
                                ...
                                if isProgSuccess then
                                    isOpening = false
                                    RemoveCrateBlip()
                                    TriggerServerEvent('jtm_randomcrate:PickUpCrate', crateID, realCrateId)
                                    ClearPedTasksImmediately(PlayerPedId())
                                else
                                    isOpening = false
                                    exports["lrp-notifications"]:Notify("error", "Je bent gestopt met de airdrop te openen!", 4000)
                                    TriggerServerEvent('jtm_randomcrate:StopOpeningCrate', realCrateId)
                                end
                            else
                                return exports["lrp-notifications"]:Notify("error", "Deze airdrop wordt al geopend!", 4000)
                            end
                        end, realCrateId)
                    end
                end
...
AddEventHandler('jtm_randomcrate:AddExplosion', function(crateID)
