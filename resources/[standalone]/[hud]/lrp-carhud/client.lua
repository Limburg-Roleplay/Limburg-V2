		...
		if IsPedInAnyVehicle(Ped, false) and showUI then
			...
			if PedCar and (GetPedInVehicleSeat(PedCar, -1) == Ped or GetPedInVehicleSeat(PedCar, -1)) then
				if IsCar(PedCar) then
					...
					if PedCar ~= 0 then -- Check if the player is in a vehicle
						local fuel = GetVehicleFuelLevel(PedCar) -- Get the fuel level of the vehicle
						SendNUIMessage({
							showfuel = true,
							fuel = fuel
						})
...
RegisterCommand('set_vehicle_mode', function(source, args, RawCommand)
	...
	if LRP.Functions.isRateLimited("lrp-carhud:change:cruise:control", 2) then
		return
	end
...
RegisterCommand('switch_vehicle_mode', function(source, args, RawCommand)
	if LRP.Functions.isRateLimited("lrp-carhud:change:vehicle:mode", 2) then
		return
	end
