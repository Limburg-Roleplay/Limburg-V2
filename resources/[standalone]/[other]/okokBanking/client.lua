...
RegisterNUICallback("action", function(data, cb)
	...
	elseif data.action == "deposit" then
		-- if ESX.GetPlayerData().playtime < 120 then
		-- 	exports['okokNotify']:Alert("Actie Mislukt", "Je moet minimaal 2 uur playtime hebben om geld te kunnen storten!", 5000, "error")
		-- 	return
		-- end
	...
	elseif data.action == "withdraw" then
		-- if ESX.GetPlayerData().playtime < 120 then
		-- 	exports['okokNotify']:Alert("Actie Mislukt", "Je moet minimaal 2 uur playtime hebben om geld te kunnen opnemen!", 5000, "error")
		-- 	return
		-- end
	...
	elseif data.action == "transfer" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				-- if ESX.GetPlayerData().playtime < 120 then
				-- 	exports['okokNotify']:Alert("Actie Mislukt", "Je moet minimaal 2 uur playtime hebben om geld te kunnen overzetten!", 5000, "error")
				-- 	return
				-- else
				-- 	exports['okokNotify']:Alert("Actie Mislukt", "Op dit moment werkt het overmaken niet. Probeer het even via je telefoon.", 5000, "error")
				-- 	return
				-- end
	...
	elseif data.action == "change_iban" then
		local alert = lib.alertDialog({
			...
			content = 'Weet je het zeker dan je je IBAN wilt aanpassen voor €50.000?',
			centered = true,
			cancel = true
		})
	...
	elseif data.action == "change_pin" then
		local alert = lib.alertDialog({
			header = 'Weet je het zeker?',
			content = 'Weet je het zeker dan je je pincode wilt aanpassen voor €1000?',
			centered = true,
			cancel = true
		})
