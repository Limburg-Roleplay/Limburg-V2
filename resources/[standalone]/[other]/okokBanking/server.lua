...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerEvent('td_logs:sendLog', 'https://discord.com/api/webhooks/1298732485944938679/yspRNc7K-uydXb4OI3RSo2Je-eyLDB5-_EWlXwglze7T6AVr-dTeasGQigLEugLfywLS', xPlayer.source, {title = "[" .. xPlayer.source .. "] " .. GetPlayerName(xPlayer.source) .. " heeft €" .. amount .. " gedeposit." , desc = "[" .. xPlayer.source .. "] " .. GetPlayerName(xPlayer.source) .. " heeft €" .. amount .. " gedeposit." }, 0x00ff00)
	else
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerEvent('td_logs:sendLog', 'https://discord.com/api/webhooks/1298732485944938679/yspRNc7K-uydXb4OI3RSo2Je-eyLDB5-_EWlXwglze7T6AVr-dTeasGQigLEugLfywLS', xPlayer.source, {title = "[" .. xPlayer.source .. "] " .. GetPlayerName(xPlayer.source) .. " heeft €" .. amount .. " opgenomen." , desc = "[" .. xPlayer.source .. "] " .. GetPlayerName(xPlayer.source) .. " heeft €" .. amount .. " opgenomen." }, 0xff0000)
	else
...
AddEventHandler("okokBanking:TransferMoney", function(amount, ibanNumber, targetIdentifier, acc, targetName)
	...
	if xPlayer.identifier ~= targetIdentifier then
		if amount <= playerMoney then
			...
			local message = "[" .. xPlayer.source .. "] " .. GetPlayerName(xPlayer.source) .. " heeft €" .. amount .. " overgemaakt naar [" .. xTarget.source .. "] " .. GetPlayerName(xTarget.source)
			TriggerEvent('td_logs:sendLog', 'https://discord.com/api/webhooks/1298732485944938679/yspRNc7K-uydXb4OI3RSo2Je-eyLDB5-_EWlXwglze7T6AVr-dTeasGQigLEugLfywLS', xPlayer.source, {title = message, desc = message }, 0x0000ff)
		else
			TriggerClientEvent('okokNotify:Alert', source, "BANK", "You don't have that much money on the bank", 5000, 'error')
		end
	else
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		...
		local message = "[" .. xPlayer.source .. "] " .. GetPlayerName(xPlayer.source) .. " heeft €" .. amount .. " overgemaakt naar " .. societyName
		TriggerEvent('td_logs:sendLog', 'https://discord.com/api/webhooks/1298732485944938679/yspRNc7K-uydXb4OI3RSo2Je-eyLDB5-_EWlXwglze7T6AVr-dTeasGQigLEugLfywLS', xPlayer.source, {title = message, desc = message }, 0x00ff00)
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "You don't have that much money on you", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	if amount <= db.value then
		if db.is_withdrawing == 1 then
			...
			local message = "[" .. xPlayer.source .. "] " .. GetPlayerName(xPlayer.source) .. " heeft €" .. amount .. " opgenomen van " .. societyName
			TriggerEvent('td_logs:sendLog', 'https://discord.com/api/webhooks/1298732485944938679/yspRNc7K-uydXb4OI3RSo2Je-eyLDB5-_EWlXwglze7T6AVr-dTeasGQigLEugLfywLS', xPlayer.source, {title = message, desc = message }, 0xff0000)
		end
	else
...
AddEventHandler("okokBanking:TransferMoneyToSocietyFromSociety", function(amount, ibanNumber, societyNameTarget, societyTarget, society, societyName, societyMoney)
	...
	if amount <= societyMoney then
		...
		local message = "[" .. xPlayer.source .. "] " .. GetPlayerName(xPlayer.source) .. " heeft €" .. amount .. " overgemaakt van " .. societyName .. " naar " .. societyNameTarget
		TriggerEvent('td_logs:sendLog', 'https://discord.com/api/webhooks/1298732485944938679/yspRNc7K-uydXb4OI3RSo2Je-eyLDB5-_EWlXwglze7T6AVr-dTeasGQigLEugLfywLS', xPlayer.source, {title = message, desc = message }, 0xff00ff)
	else
...
AddEventHandler("okokBanking:TransferMoneyToPlayerFromSociety", function(amount, ibanNumber, targetIdentifier, acc, targetName, society, societyName, societyMoney, toMyself)
	...
	if amount <= societyMoney then
		...
		local message = "[" .. xPlayer.source .. "] " .. GetPlayerName(xPlayer.source) .. " heeft €" .. amount .. " overgemaakt van " .. societyName .. " naar [" .. xTarget.source .. "] " .. GetPlayerName(xTarget.source)
		TriggerEvent('td_logs:sendLog', 'https://discord.com/api/webhooks/1298732485944938679/yspRNc7K-uydXb4OI3RSo2Je-eyLDB5-_EWlXwglze7T6AVr-dTeasGQigLEugLfywLS', xPlayer.source, {title = message, desc = message }, 0x00ffff)
	else
		TriggerClientEvent('okokNotify:Alert', source, "BANK", "Your society doesn't have that much money on the bank", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if Config.IBANChangeCost <= xPlayer.getAccount('bank').money then
		xPlayer.removeAccountMoney('bank', tonumber(Config.PINChangeCost))
		MySQL.Async.execute('UPDATE users SET iban = @iban WHERE identifier = @identifier', {
			['@iban'] = iban,
			['@identifier'] = xPlayer.identifier,
		}, function(changed)
