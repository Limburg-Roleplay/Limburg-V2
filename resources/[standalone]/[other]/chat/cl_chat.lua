...
AddEventHandler('chatMessage', function(author, ctype, text)
  ...
  local msg = '<div class="chat-message '..ctype..'"><span class="chat-message-label">{0}:</span><div class="chat-message-body">' .. text .. '</div></div>'
  SendNUIMessage({
    type = 'ON_MESSAGE',
    message = {
      template = msg,
...
AddEventHandler('chat:serverPrint', function(msg)
  print(msg)
end)

AddEventHandler('__cfx_internal:serverPrint', function(text)
  if text ~= '' then
    if not ChatActive then
      return
    end
    local msg = '<div class="chat-message warning"><span class="chat-message-label">{0}:</span><div class="chat-message-body">{1}</div></div>'
    SendNUIMessage({
      type = 'ON_MESSAGE',
      message = {
        template = msg,
...
AddEventHandler('chat:addMessage', function(message, cctype)
    ...
    local cctype = cctype ~= false and cctype or "normal"
    msg = '<div class="chat-message '..cctype..'"><span class="chat-message-label">Server:</span><div class="chat-message-body">' .. message .. '</div></div>'
    SendNUIMessage({
      type = 'ON_MESSAGE',
      message = msg
    })
