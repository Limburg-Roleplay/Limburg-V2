...
AddEventHandler('okokRequests:ShowMenu', function(sourceS, timeS, titleS, messageS, triggerS, ClientOrServerS, parametersS, parametersNumS)
	...
	if not blockrequests then
		...
		if not responded then
			...
			exports['okokNotify']:Alert("Tijd verlopen", "De maximale tijd van het verzoek is verlopen. ", 5000, 'warning')
...
RegisterNUICallback("action", function(data, cb)
	if data.action == "acceptRequest" then
		...
		exports['okokNotify']:Alert("Verzoek geaccepteerd", "Je hebt succesvol het verzoek geaccepteerd. ", 5000, 'success')
		SetNuiFocus(false, false)
	...
	elseif data.action == "declineRequest" or data.action == "closeEsc" then
		...
		exports['okokNotify']:Alert("Verzoek afgewezen", "Je hebt succesvol het verzoek afgewezen. ", 5000, 'error')
		SetNuiFocus(false, false)
...
function requestMenu(id, time, title, message, trigger, side, parameters, parametersNum)
	...
	lib.notify({
		title = 'Verzoek',
		description = 'Je hebt succesvol een verzoek verstuurd.\nWacht tot de persoon deze heeft geaccepteerd.',
		position = 'top',
		duration = 5000,
		type = 'inform'
	})
	--exports['okokNotify']:Alert("Verzoek", "Je hebt succesvol een verzoek verstuurd.<br>Wacht tot de persoon deze heeft geaccepteerd.", 5000, 'info')
end
