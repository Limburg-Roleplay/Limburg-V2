...
$(document).ready(function(){
  ...
  window.addEventListener('message', function(event){
    ...
    if(item.openBank == true) {
      $('.currentBalance').html('&euro;'+addCommas(item.bank));
      $('.username').html(item.PlayerName.firstname + " " + item.PlayerName.lastname);
      openContainer();
      openMain();
    }
  ...
  document.onkeyup = function (data) {
    if (data.which == 27 ) {
      $.post('http://frp-banking/close', JSON.stringify({}));
    }
  };
  // Handle Button Presses
  $(".btnWithdraw").click(function(){
      $.post('http://frp-banking/withdraw', JSON.stringify({}));
  });
  $(".btnDeposit").click(function(){
      $.post('http://frp-banking/deposit', JSON.stringify({}));
  });
  $(".btnBalance").click(function(){
      $.post('http://frp-banking/balance', JSON.stringify({}));
  });
  ...
  $("#withdraw-form").submit(function(e) {
      ...
      if (amount >= 0 ) {
        $.post('http://frp-banking/withdrawSubmit', JSON.stringify({
  ...
  $("#deposit-form").submit(function(e) {
      ...
      if (amount >= 0 ) {
        $.post('http://frp-banking/depositSubmit', JSON.stringify({
