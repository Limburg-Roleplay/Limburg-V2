...
LRP.closeCBR = function() {
    ...
    backendQuestion = 0;
    LRP.opened = false;
    $.post('http://lrp-rijschool/close', JSON.stringify({}));
}
...
function nextQuestion() {
    if (questions[backendQuestion+1]) {
        ...
        $.post('https://lrp-rijschool/getResults', JSON.stringify(goodAnswer), function(bool) {
            if (bool) {
                $('.image').hide();
                $('.footer').hide();
                $('.questions').hide();
