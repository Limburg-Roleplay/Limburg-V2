...
RegisterNetEvent("lrp-rijschool:client:open:theorie")
AddEventHandler(
	"lrp-rijschool:client:open:theorie",
	function(data)
		if data and data.dsad then
			ESX.TriggerServerCallback(
				"lrp-rijschool:server:has:enough:money",
				function(bool)
					if bool then
						SendNUIMessage(
							{
...
RegisterNetEvent("lrp-rijschool:client:start:rijbewijs")
AddEventHandler(
	"lrp-rijschool:client:start:rijbewijs",
	function(data)
		if data and data.type then
			ESX.TriggerServerCallback(
				...
				function(bool, plate)
					if bool then
						LRP.startLicense(data.type, plate)
					end
				end,
				"praktijk"
			)
...
openRijschoolMenu = function()
	ESX.TriggerServerCallback(
		"lrp-rijschool:server:check:license",
		function(data)
			local options = {}
			if not data then
				options[#options + 1] = {
					...
					event = "lrp-rijschool:client:open:theorie",
					args = {dsad = "hoi "}
				}
			elseif data then
				ESX.TriggerServerCallback(
					...
					function(data3)
						if not data3 then
							options[#options + 1] = {
								...
								event = "lrp-rijschool:client:start:rijbewijs",
...
RegisterNUICallback(
	...
	function(data, cb)
		if #data >= Config.GoodAnswers then
			cb(true)
			TriggerServerEvent("lrp-rijschool:server:succeed:test")
		else
			cb(false)
		end
	end
...
LRP.startLicense = function(dataType)
	local gotPoints = 0
	local number = 1
			...
			if dist <= 50.0 then
				...
				if dist <= 2.5 then
					...
					if number > maxNumber then
						TriggerServerEvent("lrp-rijschool:server:maded:praktijk:test", dataType)
						DeleteEntity(dehveh)
						if DoesBlipExist(oldblip) then
							RemoveBlip(oldblip)
						end
