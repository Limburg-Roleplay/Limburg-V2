...
AddEventHandler('exios-nonwhitelistedjobs:client:action:startJob', function(number, job)
	if job == 'technician' then
		startTechnician(number)
	end
end)

startTechnician = function(number)
	::takeVehicle::
    local taken = true
	...
	ESX.ShowNotification('success', 'Pak je voertuig en bekijk de map om de electriciteit te fixen!', 7500)
            ...
            for i=1, #Shared.Locations[number]['actions'] do
                if not Shared.Locations[number]['actions'][i].ready then
                    ...
                    if dist < 7.5 and taken and not IsPedInAnyVehicle(PlayerPedId()) then 
                        ...
                        if dist < 1.5 then 
                            exports['lrp-interaction']:Interaction({r = tostring(Shared.Locations[number]['color'].r), g = tostring(Shared.Locations[number]['color'].g), b = tostring(Shared.Locations[number]['color'].b)}, '[E] - Maak het elektra kastje.', Shared.Locations[number]['actions'][i].coords, 1.5, GetCurrentResourceName())
                            if IsControlJustPressed(0, 38) then
                                Shared.Locations[number]['actions'][i].ready = true
                                Wait(150)
                                TaskStartScenarioInPlace(PlayerPedId(), 'WORLD_HUMAN_WELDING', 0, true)
                                exports['lrp-progressbar']:Progress({
                                    name = 'exios-nonwhitelistedjobs/technician/fix',
                                    duration = 10000,
                                    label = 'Elektra Kastje Repareren',
                                    useWhileDead = false,
                                    canCancel = true,
                                    controlDisables = {
                    ...
                    if distcar < 7.5 and not taken then 
				        ...
				        if distcar <= 1.5 and not taken then
                            exports['lrp-interaction']:Interaction({r = tostring(Shared.Locations[number]['color'].r), g = tostring(Shared.Locations[number]['color'].g), b = tostring(Shared.Locations[number]['color'].b)}, '[E] Pak spullen uit voertuig', TruckCoords, 1.5, GetCurrentResourceName())
                            if IsControlJustReleased(0, 38) then
                                Wait(1)
                                taken = true
                                exports['lrp-progressbar']:Progress({
                                    name = 'exios-nonwhitelistedjobs/technician/deliver',
                                    duration = 4000,
                                    ...
                                    if not cancelled then
                                        ESX.ShowNotification('success', 'Repareer het elektra kastje.', 7500)
                                    else
                                        taken = true
                                        ClearPedTasks(PlayerPedId())
                                    end
