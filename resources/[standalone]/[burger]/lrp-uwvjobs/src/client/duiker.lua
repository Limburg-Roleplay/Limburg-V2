            ...
            for i=1, #Shared.Locations[number]['actions'] do
                if not Shared.Locations[number]['actions'][i].ready then
                    ...
                    if dist < 7.5 and taken and not IsPedInAnyVehicle(PlayerPedId()) then 
                        ...
                        if dist < 1.5 then 
                            exports['frp-interaction']:Interaction({r = tostring(Shared.Locations[number]['color'].r), g = tostring(Shared.Locations[number]['color'].g), b = tostring(Shared.Locations[number]['color'].b)}, '[E] - Repareer wrak', Shared.Locations[number]['actions'][i].coords, 1.5, GetCurrentResourceName())
                    ...
                    if distcar < 7.5 and not taken and not IsPedInAnyVehicle(PlayerPedId(), false) then 
				        ...
				        if distcar <= 1.5 then
                            ...
                            if not taken then
                                exports['frp-interaction']:Interaction({r = tostring(Shared.Locations[number]['color'].r), g = tostring(Shared.Locations[number]['color'].g), b = tostring(Shared.Locations[number]['color'].b)}, '[E] Pak spullen uit voertuig | [G] Gooi Anker in-/uit', vector3(TruckCoords.x, TruckCoords.y, TruckCoords.z + 1), 1.5, GetCurrentResourceName())
