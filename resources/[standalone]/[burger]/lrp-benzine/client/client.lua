...
Citizen.CreateThread(function()
	for k,v in pairs(Config.Settings['PumpModels']) do
		exports.qtarget:AddTargetModel(v, {
			options = {
				{
					event = 'lrp-benzine:client:pumpvehicle',
					icon = 'fas fa-gas-pump',
					label = 'Tanken'
				},
				{
					event = 'lrp-benzine:server:give:item',
					type = 'server',
					icon = 'fas fa-gas-pump',
...
RegisterNetEvent('lrp-benzine:client:not:enough:money')
AddEventHandler('lrp-benzine:client:not:enough:money', function()
	SetFuel(vehicle, fuelBeforePaying)
end)

RegisterNetEvent('lrp-benzine:client:pumpvehicle')
AddEventHandler('lrp-benzine:client:pumpvehicle', function(data)
	...
	if distance < 3.5 then
		...
		if fuelBeforePaying < 100.0 and not isFueling then
			...
			if data.jerrycan then
				TriggerServerEvent('lrp-benzine:server:take:item')
			end
			...
			while isFueling do
				...
				if not isFueling then
					...
					if ESX.PlayerData.job.name ~= 'police' and ESX.PlayerData.job.name ~= 'ambulance' and ESX.PlayerData.job.name ~= 'kmar' and ESX.PlayerData.job.name ~= 'mechanic' then
						TriggerServerEvent('lrp-benzine:server:fuel:vehicle', moneytoPay)
					else
