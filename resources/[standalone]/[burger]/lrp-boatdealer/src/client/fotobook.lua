...
RegisterNetEvent('exios-boatshop:client:handler:startFotobook')
AddEventHandler('exios-boatshop:client:handler:startFotobook', function(group)
    -- if group == 'admin' or group == 'superadmin' then
        for vehicleId = 1, #Shared.Fotobook.Vehicles do
            ...
            if vehicleLoaded then
                ESX.Game.SpawnVehicle(vehName, Shared.Fotobook.CarSettings['coords'], Shared.Fotobook.CarSettings['heading'], function(veh) 
                    ...
                    SetVehicleNumberPlateText(veh, 'Tilburg')
                    for cameraId = 1, #Shared.Fotobook.Cameras do
                        ...
                        for k,v in pairs(Shared.Fotobook.Colors) do
                            ...
                            TriggerServerEvent('exios-boatshop:server:handler:requestscreenshot', vehName, cameraId, vehBrand, k)
                            Wait(375)
                        end
        ...
        ESX.ShowNotification('success', "Foto's zijn succesvol geupload naar een development folder.")
    -- else
    --     -- Mogelijke hacker
    -- end
end)
