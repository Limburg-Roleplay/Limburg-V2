...
local function enterElevator(zone)
    ...
    for i = 1, #zone do
        for k,v in pairs(zone[i]) do
            options[k] = {value = k, label = ('Verdieping %s'):format(k)}
        end
    end

    local input = lib.inputDialog('Gebruik de lift', {
        { type = 'select', label = 'Selecteer Verdieping', options = options}
    })
    ...
    if cfg.Elevators[self.eleId] and not IsEntityDead(cache.ped) then
        ...
        elseif type(cfg.Elevators[self.eleId].jobs) == 'boolean' then
            ...
            if self.isClosest and self.currentDistance < 1.2 then
                if not hasTextUi then
                    hasTextUi = true
                    lib.showTextUI(('[E] - Gebruik %s'):format(cfg.Elevators[self.eleId].name))
                end
