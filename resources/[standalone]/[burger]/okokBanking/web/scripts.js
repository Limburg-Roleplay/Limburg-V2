...
window.addEventListener('message', function (event) {
	...
	switch (event.data.action) {
			...
			if (selectedWindow == "none") {
				$('#menu').html(`
					<div class="d-flex justify-content-center flex-column align-items-center">
						...
						<br>
						<div class="ldata-txt">Gegevens laden...</div>
					</div>
				`);
				$("#menu").fadeIn();
				$(".main_card").fadeIn();
				...
				setTimeout(function () {
					...
					$('#menu').html(`
						<div class="col-md-2 d-flex flex-column sidebar-s">
							...
							<hr>
							<span class="sidebar-title">Persoonlijk</span>
							<div id="sidebar"></div>
							<p class="sidebar-item mt-auto logout"><i class="fas fa-sign-out-alt"></i></i> <span class="ms-1">Uitloggen</span></p>
						</div>
						<div class="col-md-10 tab-s">
							<div class="d-flex justify-content-between align-items-center">
								<span class="selected-page"><span id="page-title">Overzicht</span></span>
								<div>
									<span class="username align-middle">
										<span id="playerName"></span> <span id="avatar">${avatar}</span>
									</span>
									<div class="wallet-div">
										<span>Bank: <span id="wallet_money"></span> EUR</span>
									</div>
								</div>
							</div>
							<hr>
		...
		case 'transactions_page':
			...
			$('#page-title').html('Transacties');

			if (event.data.isInSociety) {
				society = `<span class="sidebar-title mt-5">Society</span>
						   <p class="sidebar-item mt-2" id="society_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Overzicht</span></p>
						   <p class="sidebar-item" id="society_transactions"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transacties</span></p>`;
			} else {
				society = '';
			}

			$('#sidebar').html(`
				<p class="sidebar-item mt-2" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Overzicht</span></p>
				<p class="sidebar-item selected" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transacties</span></p>
				<p class="sidebar-item mt-2" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Instellingen</span></p>
				${society}
			`);
			...
			for (var i = 0; i < num; i++) {
				...
				if (db.type == 'transfer' && db.receiver_identifier == event.data.identifier) {
					...
					data = `<td class="align-middle fw500">
								Van: <span class="transactions-name">${name}</span>
								<div class="mtm3125">Ontvangen</div>
							</td>`;
					...
					icon = '<td class="align-middle"><span class="transactions-action"><i class="bi bi-download"></i></span></td>';
					data = `<td class="align-middle fw500">
								Naar: <span class="transactions-name">${name}</span>
								<div class="mtm3125">Gestort</div>
							</td>`;
					amount = `<td class="align-middle fw500 transactions-received text-center">+ ${db.value.toLocaleString()} EUR</td>`;
					// Withdrawn
					...
					data = `<td class="align-middle fw500">
								Van: <span class="transactions-name">${name}</span>
								<div class="mtm3125">Opgenomen</div>
							</td>`;
					amount = `<td class="align-middle fw500 text-center">- ${db.value.toLocaleString()} EUR</td>`;
				}
			...
			$('#page_info').html(`
				<div class="row mb-3">
					<div class="col col-md-3 pr05">
						<div class="card stats-title">
							<div class="card-body text-center">
								<h6 class="card-title">Transacties</h6>
					...
					<div class="col col-md-3 pr05 pl05">
						<div class="card stats-title">
							<div class="card-body text-center">
								<h6 class="card-title">Inkomsten</h6>
					...
					<div class="col col-md-3 pr05 pl05">
						<div class="card stats-title">
							<div class="card-body text-center">
								<h6 class="card-title">Uitgaven</h6>
					...
					<div class="col col-md-3 pl05">
						<div class="card stats-title">
							<div class="card-body text-center">
								<h6 class="card-title">Verdiensten</h6>
			...
			table.push(new simpleDatatables.DataTable(table_id, {
				...
				labels: {
					placeholder: "Zoeken...",
					searchTitle: "Zoeken in tabel",
					pageTitle: "Pagina {page}",
					perPage: "transacties per pagina",
					noRows: "Geen transacties gevonden",
					info: "Toont {start} tot {end} van {rows} resultaten",
		...
		case 'society_transactions':
			...
			if (event.data.isInSociety) {
				society = `<span class="sidebar-title mt-5">Society</span>
						   <p class="sidebar-item mt-2" id="society_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Overzicht</span></p>
						   <p class="sidebar-item selected" id="society_transactions"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transacties</span></p>`;
			} else {
				society = '';
			}
			...
			$('#sidebar').html(`
				<p class="sidebar-item mt-2" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Overzicht</span></p>
				<p class="sidebar-item" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transacties</span></p>
				<p class="sidebar-item mt-2" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Instellingen</span></p>
				${society}
			`);
...
function overview_page_function(event) {
	if (event.data.isUpdate && selectedWindow == "bankmenu" || !event.data.isUpdate) {
		...
		$('#sidebar').html(`
			<p class="sidebar-item mt-2 selected" id="Overzicht"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Overzicht</span></p>
			<p class="sidebar-item" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transacties</span></p>
			<p class="sidebar-item mt-2" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Instellingen</span></p>
			${society}
		`);
		...
		if (event.data.RequireCC) {
			informations = `<span class="card-o-title">Informatie<span class="badge bg-primary viewall-badge buy_new_card" data-bs-toggle="modal" data-bs-target="#buycc_modal"><i class="fa-solid fa-plus"></i> NEW</span></span>`;
		} else {
			informations = `<span class="card-o-title">Informatie</span>`;
		}
			...
			<div class="col-md-7 chart-main">
				<div class="card chart-card text-center">
					<div class="card-header card-o-header">
						<span class="card-o-title">Statistieken</span>
					</div>
					...
					<div class="col-md-12">
						<div class="card last-t-card">
							<div class="card-header card-o-header text-center">
								<span class="card-o-title">Laatste Transacties<span class="badge bg-primary viewall-badge" id="view_all_transactions"><i class="fas fa-eye"></i> Alles bekijken</span></span>
							</div>
			...
			<div class="col-md-5">
				<div class="col-md-12">
					...
					<div class="card text-center h-100 actions-card">
						<div class="card-header card-o-header">
							<span class="card-o-title">Acties</span>
						</div>
						<div class="card-body actions-card_body">
							<div class="d-flex justify-content-center">
								<button type="button" id="depositMoneyModal" class="btn btn-blue flex-grow-1" data-bs-toggle="modal" data-bs-target="#depositModal"><i class="bi bi-upload"></i> Storten</button>
							</div>
							<div class="d-flex justify-content-center mt4375">
								<button type="button" id="withdrawMoneyModal" class="btn btn-blue flex-grow-1" data-bs-toggle="modal" data-bs-target="#withdrawModal"><i class="bi bi-download"></i> Opnemen</button>
							</div>
							<div class="d-flex justify-content-center mt4375">
								<button type="button" id="transferMoneyModal" class="btn btn-blue flex-grow-1" data-bs-toggle="modal" data-bs-target="#transferModal"><i class="fas fa-exchange-alt"></i> Overmaken</button>
							</div>
						</div>
					</div>
				</div>
		...
		for (var i = 0; i < num; i++) {
			...
			if (db.type == 'transfer' && db.receiver_identifier == event.data.identifier) {
				...
				data = `<td class="align-middle lastT-name-div">
							Van: <span class="lastT-name">${name}</span>
							<div class="mtm3125">Ontvangen</div>
						</td>`;
				...
				icon = '<td class="align-middle"><span class="lastT-action"><i class="bi bi-download"></i></span></td>';
				data = `<td class="align-middle lastT-name-div">
							Naar: <span class="lastT-name">${name}</span>
							<div class="mtm3125">Gestort</div>
						</td>`;
				amount = `<td class="align-middle fw500 lastT-received"><span class="floatr">+ ${db.value.toLocaleString()} EUR</span></td>`;
				// Withdrawn
...
function society_page_function(event) {
	if (event.data.isUpdate && selectedWindow == "societies" || !event.data.isUpdate) {
		...
		if (event.data.isInSociety) {
			society = `<span class="sidebar-title mt-5">Society</span>
					   <p class="sidebar-item mt-2 selected" id="society_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Overzicht</span></p>
					   <p class="sidebar-item" id="society_transactions"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transacties</span></p>`;
		} else {
			society = '';
		}
		...
		$('#page_info').html(`
			<div class="col-md-7 chart-main">
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card last-t-card">
							<div class="card-header card-o-header text-center">
								<span class="card-o-title">Laatste Transacties<span class="badge bg-primary viewall-badge" id="view_all_transactions_society"><i class="fas fa-eye"></i> Alles bekijken</span></span>
							</div>
							<div class="card-body" id="last-t-body">
								<table id="lastTransactionsTable">
									<tbody id="lastTransactionsData">
			...
			<div class="col-md-5">
				<div class="col-md-12">
					<div class="card ccard-card">
						<div class="card-header card-o-header text-center">
							<span class="card-o-title">Informatie</span>
						</div>
						<div class="card-body ccard-body">
							<div class="card creditcard-classic_card d-flex align-items-center">
								<div class="card-body creditcard-classic_card-body">
									<span class="d-flex justify-content-between"><span><span class="ccard-name">Limburg Bank</span></span><span><i class="fas fa-wifi"></i></span></span>
									<div class="mt38p">
										<div class="d-flex align-items-center">
											<span class="ccard-status">Status</span>
										</div>
...
function settings_page_function(event) {
	...
	$('#sidebar').html(`
		<p class="sidebar-item mt-2" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Overzicht</span></p>
		<p class="sidebar-item" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transacties</span></p>
		<p class="sidebar-item mt-2 selected" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Instellingen</span></p>
		${society}
	`);
						...
						<div class="row">
							<div class="col col-md-6 d-flex justify-content-center">
								<div class="card w-100 h-100 changeiban-card">
									<div class="card-body text-center">
										<span class="fs15 fff">IBAN Wijzigen</span>
										<input type="text" maxlength="${event.data.ibanCharNum}" id="new_iban" class="form-control text-center" placeholder="Nieuwe IBAN" onkeyup="checkIfEmptySettings()">
										<button type="button" id="change_iban" class="btn btn-blue" disabled><i class="fas fa-paper-plane"></i> Verzenden</button>
									</div>
								</div>
							</div>
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center settings_info-card">
									<div class="card-body text-center fs1125">
										<span>Het wijzigen van het IBAN kost ${event.data.ibanCost}€</span>
										<hr class="fff">
										<span>Het IBAN heeft altijd een voorvoegsel "${event.data.ibanPrefix}"</span>
										<hr class="fff">
										<span>Het maximale aantal tekens is ${event.data.ibanCharNum}</span>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col col-md-6 d-flex justify-content-center">
								<div class="card w-100 h-100 changepin-card">
									<div class="card-body text-center">
										<span class="fs15 fff">PIN Wijzigen</span>
										<input type="password" maxlength="4" id="new_pin" class="form-control text-center" placeholder="Nieuwe PIN" onkeyup="checkIfEmptySettings()">
										<button type="button" id="change_pin" class="btn btn-blue" disabled><i class="fas fa-paper-plane"></i> Verzenden</button>
									</div>
								</div>
							</div>
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center settings_info-card">
									<div class="card-body text-center fs1125">
										<span>Het wijzigen van de PIN kost ${event.data.pinCost}€</span>
										<hr class="fff">
										<span>Het maximale aantal tekens is ${event.data.pinCharNum}</span>
										<hr class="fff">
										<span>Je kunt alleen cijfers gebruiken</span>
									</div>
								</div>
							</div>
						</div>
	...
	setTimeout(function () {
		$('#menu').html(`
			<div class="d-flex justify-content-center flex-column align-items-center">
				<span class="load"></span>
				<br>
				<div class="ldata-txt">Uitloggen...</div>
			</div>
		`);
		$("#menu").fadeIn();
		setTimeout(function () {
