...
RegisterNetEvent('lrp-carkeys:client:unl:vehicle')
AddEventHandler('lrp-carkeys:client:unl:vehicle', function(status, entity)
...
Exios.Functions.LockVeh = function()
    ...
    if veh ~= 0 and dist < 5.0 then
        ...
        ESX.TriggerServerCallback('lrp-carkeys:server:can:unlock:vehicle', function(bool, error)
...
exports('giveCarKeys', function(src, plate, props)
	TriggerServerEvent('lrp-carkeys:server:add:carKey', src, plate, props)
end)
