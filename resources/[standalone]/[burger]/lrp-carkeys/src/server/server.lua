...
RegisterNetEvent('lrp-carkeys:server:add:carKey')
AddEventHandler('lrp-carkeys:server:add:carKey', function(plate, props)
    exports[''..Shared.Carkeys..'']:addCarKey(source, plate, props)
end)

-- // [CALLBACKS] \\ --

ESX.RegisterServerCallback('lrp-carkeys:server:can:unlock:vehicle', function(source, cb, plate)
...
-- // [/GEEFSLEUTELS] \\ --
    ...
    AddEventHandler('esx_givecarkeys:setVehicleOwnedPlayerId', function (playerId, vehicleProps)
        ...
        function (rowsChanged)
            TriggerClientEvent("lrp-notifications:client:notify", xPlayer.playerId, 'success', 'U heeft een nieuwe auto met kenteken ' ..vehicleProps.plate..'')
        end)
    end)
