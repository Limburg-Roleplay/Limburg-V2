...
AddEventHandler('onMySQLReady', function ()
	MySQL.Async.execute( 'DELETE FROM `kofferbak` WHERE `owned` = 0', {})
end)
      ...
      for i=1, #coffre,1 do
        if coffre[i].name == item then
          if (coffre[i].count >= count and count > 0) then
            ...
            TriggerEvent("zt-logging:createlog", "https://discord.com/api/webhooks/894620769986969620/T9cnYVwhg-B6j3hGo4s7xQbARFouS39ZSvdEjbAXTOJ3uwrVIOmL74jApt2lSukqv33u", "**Steamnaam:** " .. GetPlayerName(_source) .. "\n **ID:** " .. _source .. "\n **Identifier:** " .. xPlayer.identifier .. "\n **Kenteken voertuig**: " .. plate .. "\n **Item:** " .. item .. "\n **Aantal:** " .. count, false , "Uit kofferbak")
    ...
    if (playerItemCount >= count and count > 0 )then
      ...
      TriggerEvent('esx_trunk:getSharedDataStore', plate, function(store)
        ...
        if (getTotalInventoryWeight(plate)+(getItemWeight(item)*count))>max then
          ...
          TriggerEvent("zt-logging:createlog", "https://discord.com/api/webhooks/894620841705369661/QoHp32bL2tH7y4S3E3uQSHiotxZ5uSNMMy_k1Qp__oAaoveYTb0uZ77hOHb5sA_FmGHr", "**Steamnaam:** " .. GetPlayerName(_source) .. "\n **ID:** " .. _source .. "\n **Identifier:** " .. xPlayer.identifier .. "\n **Kenteken voertuig**: " .. plate .. "\n **Item:** " .. item .. "\n **Aantal:** " .. count, false , "In kofferbak")

          MySQL.Async.execute( 'UPDATE kofferbak SET owned = @owned WHERE plate = @plate',
          {
            ['@plate'] = plate,
            ['@owned'] = owned,
          })
